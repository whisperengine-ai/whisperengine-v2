...
async def test_conversation_analysis():
    ...
    for case in test_conversations:
        ...
        if "expected_support_seeking" in case and case["expected_support_seeking"] != analysis.support_seeking:
            status = "❌"
        
        if "expected_knowledge_sharing" in case and case["expected_knowledge_sharing"] != analysis.knowledge_sharing:
            status = "❌"
...
async def test_personality_summary(profiler, user_id):
    ...
    if summary["personality_traits"]:
        ...
        for trait_name, trait_data in summary["personality_traits"].items():
            print(f"   {trait_name}: {trait_data['value']:.2f} (confidence: {trait_data['confidence']:.2f})")
        print()
    ...
    if summary["confidence_metrics"]:
        ...
        print(f"   High confidence traits: {metrics['traits_high_confidence']}/{metrics['traits_total']}")
        print()
...
async def test_real_time_adaptation():
    ...
    print(f"   Formality preference: {profile1.preferred_response_style.get('formality', 'none set')}")
    print(f"   AI recommendation: {recommendations1.get('communication_style', {}).get('formality', 'none')}")
    print()
    ...
    print(f"   Support strategy: {recommendations2.get('support_strategy', {}).get('approach', 'none')}")
    print()
    ...
    print(f"   Conversation tactics: {recommendations3.get('conversation_tactics', {}).get('depth', 'none')}")
    print()
