...
class CriticalPathTester:
    ...
    async def test_memory_moments_critical_paths(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Prompt integration: {len(prompt) if prompt else 0} characters generated"
            )
    ...
    async def test_thread_manager_critical_paths(self):
        ...
        try:
            ...
            if user_threads:
                ...
                await self.framework.thread_manager.pause_thread(
                    test_user, thread_id, "User request"
                )
    ...
    async def test_engagement_engine_critical_paths(self):
        ...
        try:
            ...
            recommendations = (
                await self.framework.engagement_engine.generate_conversation_recommendations(
...
class ConcurrencyTester:
    ...
    async def test_concurrent_memory_operations(
        self, user_count: int = 50, operation_count: int = 20
    ):
        """Test memory moments system under concurrent load"""
        logger.info(
            f"🧠 Testing concurrent memory operations: {user_count} users, {operation_count} ops each"
        )
        ...
        async def memory_operation_worker(user_id: str, operation_id: int):
            try:
                context = ConversationContext(
                    ...
                    emotional_state=(
                        EmotionalState.CURIOSITY if EMOTIONAL_CONTEXT_AVAILABLE else None
                    ),
                )
        ...
        logger.info(
            f"✅ Memory concurrency test: {successful_ops}/{total_ops} ops successful "
    ...
    async def test_concurrent_thread_management(
        self, user_count: int = 30, message_count: int = 10
    ):
        """Test thread manager system under concurrent load"""
        logger.info(
            f"🧵 Testing concurrent thread management: {user_count} users, {message_count} messages each"
        )
        ...
        logger.info(
            f"✅ Thread concurrency test: {successful_ops}/{total_ops} ops successful "
            f"({concurrency_result['success_rate']:.2%}) in {duration:.2f}s, "
    ...
    async def test_concurrent_engagement_analysis(
        self, user_count: int = 40, analysis_count: int = 15
    ):
        """Test engagement engine under concurrent load"""
        logger.info(
            f"💡 Testing concurrent engagement analysis: {user_count} users, {analysis_count} analyses each"
        )
        ...
        total_interventions = sum(
            1 for r in results if isinstance(r, dict) and r.get("intervention_needed", False)
        )
        ...
        logger.info(
            f"✅ Engagement concurrency test: {successful_ops}/{total_ops} ops successful "
            f"({concurrency_result['success_rate']:.2%}) in {duration:.2f}s, "
...
async def test_phase4_critical_paths():
    ...
    assert all(
        engagement_results.values()
    ), f"Engagement engine critical path failures: {engagement_results}"
...
async def test_phase4_concurrency():
    ...
    assert (
        memory_concurrency["success_rate"] >= 0.95
    ), f"Memory concurrency test failed: {memory_concurrency}"

    # Test concurrent thread management
    thread_concurrency = await concurrency_tester.test_concurrent_thread_management(30, 10)
    assert (
        thread_concurrency["success_rate"] >= 0.95
    ), f"Thread concurrency test failed: {thread_concurrency}"
    ...
    assert (
        engagement_concurrency["success_rate"] >= 0.95
    ), f"Engagement concurrency test failed: {engagement_concurrency}"
