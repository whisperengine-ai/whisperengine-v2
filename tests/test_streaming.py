...
async def test_streaming_vs_regular():
    ...
    try:
        ...
        print(f"✅ Streaming generator: {len(streaming_generator_audio)} bytes in {total_streaming_time:.2f} seconds")
        print(f"⚡ Time to first chunk: {first_chunk_time:.2f} seconds ({len(chunks)} total chunks)")
        ...
        if len(regular_audio) == len(streaming_audio) == len(streaming_generator_audio):
            ...
            print(f"⚠️  Audio size mismatch: regular={len(regular_audio)}, streaming={len(streaming_audio)}, generator={len(streaming_generator_audio)}")
