...
async def test_desktop_app_startup():
    ...
    try:
        ...
        if init_success:
            ...
            print(
                f"   📊 Vector storage: {health.get('vector_storage', {}).get('status', 'unknown')}"
            )
            print(
                f"   📊 Graph storage: {health.get('graph_storage', {}).get('status', 'unknown')}"
            )
            ...
            print(
                f"   📊 Vector collections: {len(stats.get('vector_storage', {}).get('collections', {}))}"
            )
            print(
                f"   📊 Graph nodes: {stats.get('graph_storage', {}).get('graph_statistics', {}).get('nodes', 0)}"
            )
