...
class TestMemoryTriggeredMoments:
    ...
    def memory_system(self):
        ...
        return MemoryTriggeredMoments(
            connection_retention_days=30, max_connections_per_user=100, moment_cooldown_hours=12
        )
    ...
    def sample_conversation_context(self, sample_user_id):
        ...
        return ConversationContext(
            ...
            recent_messages=[
                "Hello!",
                "How are you?",
                "I've been working on my programming skills lately",
            ],
    ...
    async def test_analyze_conversation_for_memories_basic(self, memory_system, sample_user_id):
        ...
        assert any(
            conn.connection_type == MemoryConnectionType.THEMATIC_SIMILARITY
        ...
        thematic_conn = next(
            conn
            for conn in connections2
            if conn.connection_type == MemoryConnectionType.THEMATIC_SIMILARITY
        )
    ...
    async def test_emotional_resonance_connections(self, memory_system, sample_user_id):
        ...
        emotional_connections = [
            ...
            if conn.connection_type == MemoryConnectionType.EMOTIONAL_RESONANCE
        ]
    ...
    async def test_memory_moment_generation(
        self, memory_system, sample_user_id, sample_conversation_context
    ):
        ...
        moments = await memory_system.generate_memory_moments(
            sample_user_id, sample_conversation_context
        )
    ...
    async def test_moment_appropriateness_filtering(self, memory_system, sample_user_id):
        ...
        moments = await memory_system.generate_memory_moments(
            sample_user_id, low_relationship_context
        )
    ...
    async def test_memory_moment_triggering(
        ...
        moment = moments[0]
        callback_text = await memory_system.trigger_memory_moment(
            moment, sample_conversation_context
        )
        ...
        callback_text_2 = await memory_system.trigger_memory_moment(
            moment, sample_conversation_context
        )
    ...
    def test_moment_type_determination(self, memory_system, sample_conversation_context):
        ...
        moment_type = memory_system._determine_moment_type(
            achievement_connection, sample_conversation_context
        )
        ...
        moment_type = memory_system._determine_moment_type(
            emotional_connection, sample_conversation_context
        )
        assert (
            moment_type == MemoryMomentType.CELEBRATION_ECHO
        )  # Since conversation context has JOY
    ...
    def test_callback_text_generation(self, memory_system):
        ...
        callback = memory_system._generate_callback_text(
            connection, MemoryMomentType.ACHIEVEMENT_CALLBACK
        )
        ...
        callback = memory_system._generate_callback_text(
            connection, MemoryMomentType.INTEREST_DEVELOPMENT
        )
        assert "passionate" in callback.lower() or "topic" in callback.lower()

    @pytest.mark.asyncio
    async def test_memory_moment_prompt_generation(
        self, memory_system, sample_user_id, sample_conversation_context
    ):
        ...
        if moments:  # Only test if we have moments
            prompt = await memory_system.get_memory_moment_prompt(
                moments, sample_conversation_context
            )
