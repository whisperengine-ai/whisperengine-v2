...
async def test_user_specific_filtering():
    ...
    print("🧪 Testing user-specific conversation filtering...")
    ...
    messages = [
        ...
        create_mock_message(bot_user, "The weather today is sunny, Alice", is_bot=True, message_id=6),
    ...
    print(f"  Added {len(messages)} messages from 3 users + bot to cache")
    ...
    assert len(alice_user_messages) == 2, f"Alice should see 2 of her own messages, got {len(alice_user_messages)}"
    assert len(alice_bot_messages) >= 2, f"Alice should see bot responses, got {len(alice_bot_messages)}"  
    assert len(other_user_messages) == 0, f"Alice should NOT see other users' messages, got {len(other_user_messages)}"
    ...
    assert "I want to know about the weather" in alice_contents, "Alice should see her weather question"
    
    # Verify Alice does NOT see other users' secrets
    all_alice_content = " ".join([msg.content for msg in alice_context])
    assert "Secret message from Charlie" not in all_alice_content, "Alice should NOT see Charlie's secret"
    assert "Hey bot, I'm Bob" not in all_alice_content, "Alice should NOT see Bob's messages"
    
    print("  ✅ Alice's context is properly filtered")
    ...
    assert len(bob_user_messages) == 2, f"Bob should see 2 of his own messages, got {len(bob_user_messages)}"
    assert len(other_user_messages_bob) == 0, f"Bob should NOT see other users' messages, got {len(other_user_messages_bob)}"
    ...
    assert "Secret message from Charlie" not in all_bob_content, "Bob should NOT see Charlie's secret"
    assert "weather" not in all_bob_content, "Bob should NOT see Alice's weather question"
    
    print("  ✅ Bob's context is properly filtered")
    
    # Test Charlie's conversation context (should only see his own message)
    charlie_context = await cache.get_user_conversation_context(mock_channel, user_charlie, limit=10)
    
    print(f"  Charlie's context contains {len(charlie_context)} messages:")
    ...
    other_user_messages_charlie = [msg for msg in charlie_context if not msg.author.bot and msg.author.id != user_charlie]
    
    assert len(charlie_user_messages) == 1, f"Charlie should see 1 of his own messages, got {len(charlie_user_messages)}"
    assert len(other_user_messages_charlie) == 0, f"Charlie should NOT see other users' messages, got {len(other_user_messages_charlie)}"
    
    # Verify Charlie's message content
    assert charlie_user_messages[0].content == "Secret message from Charlie", "Charlie should see his secret message"
    ...
    print("  ✅ Charlie's context is properly filtered")
    
    print("✅ User-specific filtering test passed")

async def test_dm_security():
    """Test security in DM context where cache mixing is still possible"""
    
    print("\n🧪 Testing DM conversation security...")
    ...
    alice_messages = [
        ...
        create_mock_message(bot_user, "Tell me more about your plan, Alice", is_bot=True, message_id=102),
        create_mock_message(user_alice, "I want to surprise my friend with a party", message_id=103),
        create_mock_message(bot_user, "That sounds wonderful! Need help planning?", is_bot=True, message_id=104),
    ]
    
    # Add messages to Bob's DM  
    bob_messages = [
        create_mock_message(user_bob, "Hey bot, I need help with work", message_id=201),
        create_mock_message(bot_user, "What kind of work help do you need, Bob?", is_bot=True, message_id=202),
        create_mock_message(user_bob, "I'm struggling with my presentation tomorrow", message_id=203),
    ]
    ...
    print(f"  Added {len(alice_messages)} messages to Alice's DM")
    print(f"  Added {len(bob_messages)} messages to Bob's DM")
    
    # Test Alice's DM context - should only see Alice + bot from her DM
    alice_dm_context = await cache.get_user_conversation_context(alice_dm_channel, user_alice, limit=10)
    
    print(f"  Alice's DM context contains {len(alice_dm_context)} messages:")
    ...
    print(f"  Bob's DM context contains {len(bob_dm_context)} messages:")
...
async def test_bot_response_filtering():
    ...
    messages = [
        ...
        create_mock_message(bot_user, "The capital of France is Paris, Bob!", is_bot=True, message_id=304),
        create_mock_message(user_alice, "Thanks for the math help!", message_id=305),
        create_mock_message(bot_user, "You're welcome, Alice! Need more help?", is_bot=True, message_id=306),
    ]
    ...
    print(f"  Added {len(messages)} messages with interleaved bot responses")
    ...
    print("  ✅ Bot responses are included but user messages are properly filtered")
    print("✅ Bot response filtering test passed")
...
async def test_performance_and_limits():
    ...
    for i in range(100):  # 100 messages
        if i % 2 == 0:
            messages.append(create_mock_message(user_alice, f"Alice message {i}", message_id=1000+i))
        else:
            messages.append(create_mock_message(bot_user, f"Bot response {i}", is_bot=True, message_id=1000+i))
    ...
    print(f"  Added {len(messages)} messages in {add_time:.3f} seconds")
    ...
    print(f"  Retrieved user context in {retrieval_time:.3f} seconds")
    print(f"  Context contains {len(alice_context)} messages (limit was 10)")
    ...
    print("✅ Performance and limits test passed")
...
async def test_edge_cases():
    ...
    print("  ✅ Empty channel handled correctly")
    ...
    alice_context_empty = await cache.get_user_conversation_context(other_users_channel, user_alice, limit=5)
    assert len(alice_context_empty) == 0, "Alice should see no messages from other users"
    print("  ✅ Other users only channel handled correctly")
    ...
    alice_bot_context = await cache.get_user_conversation_context(bot_only_channel, user_alice, limit=5)
    # Alice should still see bot messages even if she hasn't sent any
    assert len(alice_bot_context) == 2, f"Alice should see bot messages, got {len(alice_bot_context)}"
    print("  ✅ Bot-only channel handled correctly")
    
    print("✅ Edge cases test passed")

if __name__ == "__main__":
    print("🔒 Conversation Cache Mixing Security Fix - Test Suite")
    print("=" * 65)
    ...
    async def run_all_tests():
        try:
            ...
            print("  ✅ DM conversation isolation")
            print("  ✅ Bot response filtering")
            print("  ✅ Performance and limits")
            ...
            print("  ✅ Channel-based cache segmentation fixed") 
            print("  ✅ User-level conversation isolation enforced")
            
            print("\n✅ Conversation Cache Mixing (CVSS 7.2) - SECURITY FIX VALIDATED")
