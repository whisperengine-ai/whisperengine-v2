...
from debug_mode_security import SecureDebugLogger, secure_add_debug_info_to_response, test_secure_debug_logging
...
def test_sensitive_data_sanitization():
    ...
    for original, expected in test_cases:
        ...
        assert expected == sanitized, f"Failed to sanitize: {original} -> {sanitized}, expected: {expected}"
...
def test_username_sanitization():
    ...
    test_cases = [
        ...
        ("very_long_username@example.com", "ve***************************m"),  # 30 chars, show first 2, last 1, mask middle 27
    ]
    
    for original, expected in test_cases:
        sanitized = debug_logger.sanitize_username(original)
        assert sanitized == expected, f"Username sanitization failed: {original} -> {sanitized}, expected: {expected}"
...
def test_production_mode_security():
    ...
    assert sanitized == "[username_hidden]", f"Production mode should hide usernames, got: {sanitized}"
    ...
    assert "details hidden in production" in log_output or log_output == "", "Production mode should limit debug output"
...
def test_debug_function_integration():
    ...
    assert secure_response_dup == response, "Duplicate processing should still return clean response"
...
def test_log_level_security():
    ...
    assert "VeryPrivateUser@secret.com" not in all_output, "Sensitive email should not appear in logs"
    assert "Ve***********************m" in all_output or "[username_hidden]" in all_output, "Username should be sanitized"
