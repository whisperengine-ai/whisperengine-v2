...
class JakeE2EValidator:
    ...
    async def validate_qdrant_memory_with_roberta(self) -> bool:
        ...
        try:
            ...
            if roberta_found > 0:
                ...
                print(f"{YELLOW}⚠️  No RoBERTa metadata found{RESET}")
                return True  # Not fatal
        ...
        if messages:
            ...
            if system_msg:
                ...
                if 'Jake' in content or 'photographer' in content.lower() or 'adventure' in content.lower():
                    print(f"{GREEN}✅ CDL character integration detected (Jake){RESET}")
                else:
    ...
    async def validate_docker_logs(self) -> bool:
        ...
        checks = {
            'relationship_update': '🔄 Relationship updated' in logs or 'relationship' in logs.lower(),
    ...
    async def run_full_validation(self):
        ...
        try:
            ...
            if all_passed:
                ...
                print(f"{YELLOW}⚠️  Some tests failed - Sprint 3 needs message processor integration{RESET}")
            print(f"{CYAN}{'='*80}{RESET}\n")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    print(f"{RED}❌ Jake bot not healthy on port 9097{RESET}")
                    print(f"{YELLOW}Run: ./multi-bot.sh start jake{RESET}")
                    return False
                print(f"{GREEN}✅ Jake bot is healthy{RESET}")
    except Exception as e:
        print(f"{RED}❌ Cannot connect to Jake bot: {e}{RESET}")
        print(f"{YELLOW}Run: ./multi-bot.sh start jake{RESET}")
        return False
