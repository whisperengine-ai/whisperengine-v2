...
def test_voice_max_response_length():
    ...
    ), f"Response not properly truncated"  # +3 for "..."
    assert truncated_response.endswith("..."), "Truncated response should end with '...'"
    print(
        f"✓ Test 5 passed: Response truncation works (original: {len(test_response)} chars, truncated: {len(truncated_response)} chars)"
    )
