...
async def test_thread_manager_concurrency():
    ...
    logger.info(
        f"✅ Thread manager: {successful_ops}/{total_ops} ops successful "
...
async def test_memory_moments_concurrency():
    ...
    logger.info(
        f"✅ Memory moments: {successful_ops}/{total_ops} ops successful "
        f"({successful_ops/total_ops:.1%}) in {duration:.2f}s"
    )
...
async def test_engagement_engine_concurrency():
    ...
    logger.info(
        f"✅ Engagement engine: {successful_ops}/{total_ops} ops successful "
        f"({successful_ops/total_ops:.1%}) in {duration:.2f}s"
    )
...
async def main():
    ...
    for result in results:
        ...
        if "error" in result:
            ...
            logger.info(
                f"{status} {component:20} - {success_rate:6.1%} success, {ops_per_sec:6.1f} ops/sec"
            )
