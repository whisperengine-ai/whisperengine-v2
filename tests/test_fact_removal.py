...
def test_fact_removal():
    ...
    try:
        ...
        cat_facts = [m for m in memories if m['metadata'].get('type') == 'user_fact' and 'cat' in m['metadata'].get('fact', '').lower()]
        ...
        if cat_facts:
            ...
            if memory_manager.delete_specific_memory(cat_fact['id']):
                ...
                # Verify it's gone
                memories_after = memory_manager.retrieve_relevant_memories(test_user_id, "cat", limit=10)
                cat_facts_after = [m for m in memories_after if m['metadata'].get('type') == 'user_fact' and 'cat' in m['metadata'].get('fact', '').lower()]
