...
def test_personality_fact_integration():
    ...
    try:
        ...
        print(f"   📊 Relevance: {personality_fact.relevance.value} ({personality_fact.relevance_score:.2f})")
        ...
        retrieved_facts = memory_manager.retrieve_personality_facts(
        ...
        new_facts = memory_manager.retrieve_personality_facts(
            user_id=test_user_id,
            limit=20
        )
        ...
        for fact in new_facts[1:]:  # Skip the first one we manually added
            print(f"   🆕 Auto-extracted: {fact.get('fact_type', 'unknown')} - {fact.get('content', 'Unknown')[:50]}...")
