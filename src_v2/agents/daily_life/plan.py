...
def build_plan_prompt(state: DailyLifeState, character: "Character") -> str:
    ...
    # Check settings
    can_post = getattr(settings, "ENABLE_AUTONOMOUS_POSTING", False)
...
- "react" - Add emoji reaction (requires channel_id, target_message_id, emoji){
    f'\n- "post" - Start a new conversation (requires channel_id). Can optionally set target_bot_name to mention another bot.' if can_post else ''
}
