...
Be helpful, engaging, and demonstrate your advanced capabilities. Keep responses conversational but informative."""
    ...
    def delete_custom_prompt(self, name: str) -> bool:
        ...
        try:
            if name in self.settings["system_prompt"]["custom_prompts"]:
                ...
                if (self.settings["system_prompt"]["prompt_source"] == "custom" and 
                    self.settings["system_prompt"]["active_prompt"] == name):
    ...
    def get_uploaded_prompts(self) -> List[str]:
        ...
        try:
            return [f.name for f in self.system_prompts_dir.glob("*.txt") 
                   if f.is_file()]
        except Exception as e:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get("https://discord.com/api/v10/users/@me", 
                    ...
                    if response.status == 200:
                        ...
                        self.settings["discord_config"]["last_validated"] = datetime.now().isoformat()
