...
class LocalGraphStorage:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for edge in edges:
                    ...
                    self.edge_index[edge['relationship_type']].add((edge['source_id'], edge['target_id']))
    ...
    async def create_or_update_user(self, user_id: str, discord_id: str, 
                                   ...
                                   avatar_url: str = "", **kwargs) -> Dict[str, Any]:
    ...
    async def create_memory_with_relationships(self, memory_id: str, user_id: str,
        ...
        with self._lock:
            ...
            for topic_id in (topics or []):
                await self._create_relationship(
                    memory_id, topic_id, 'RELATES_TO',
                    {'relevance': 1.0, 'created_at': now}
                )
            ...
            for related_memory_id in (related_memories or []):
                if self.graph.has_node(related_memory_id):
                    await self._create_relationship(
                        memory_id, related_memory_id, 'RELATED_TO',
                        {'strength': 0.8, 'created_at': now}
                    )
            
            await self._persist_node(memory_id, 'Memory', properties, now)
            
            return {'memory_id': memory_id, 'relationships_created': len(topics or []) + len(related_memories or []) + 1}
    ...
    async def _create_relationship(self, source_id: str, target_id: str,
        ...
        # Persist to SQLite
        await self._persist_edge(edge_id, source_id, target_id, relationship_type, properties, weight)
    ...
    async def get_user_relationship_context(self, user_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            memories = [node for node in connected_nodes 
                       if self.graph.nodes.get(node, {}).get('label') == 'Memory']
            
            # Topic analysis
            topics = set()
            for memory in memories:
                topic_neighbors = [n for n in self.graph.neighbors(memory)
            ...
            for topic_node in topic_nodes:
                try:
                    ...
                    for path in paths:
                        ...
                        for node in path:
                            if (self.graph.nodes.get(node, {}).get('label') == 'Memory' and 
                                node not in [m['memory_id'] for m in contextual_memories]):
                                
                                memory_data = {
                                    'memory_id': node,
                                    'content': self.graph.nodes[node]['properties'].get('content', ''),
                                    'importance': self.graph.nodes[node]['properties'].get('importance', 0.5),
    ...
    async def get_emotional_patterns(self, user_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for memory in user_memories:
                emotional_context = self.graph.nodes[memory]['properties'].get('emotional_context', '')
    ...
    async def _persist_node(self, node_id: str, label: str, properties: Dict[str, Any], timestamp: str):
    ...
    async def _persist_edge(self, edge_id: str, source_id: str, target_id: str,
                           relationship_type: str, properties: Dict[str, Any], weight: float):
        """Persist edge to SQLite"""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                INSERT OR REPLACE INTO edges (id, source_id, target_id, relationship_type, properties, weight, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (edge_id, source_id, target_id, relationship_type, json.dumps(properties), weight, datetime.now().isoformat()))
            conn.commit()
