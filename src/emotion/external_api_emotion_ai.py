...
class ExternalAPIEmotionAI:
    ...
    async def analyze_emotion_cloud(self, 
                                  text: str, 
    ...
    async def _analyze_full_capabilities(self, text: str, user_id: Optional[str] = None, conversation_history: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        if conversation_history and user_id and len(conversation_history) > 1:
            context_emotion = await self._analyze_conversation_context_heavy(user_id, conversation_history)
        ...
        intensity = self._calculate_emotional_intensity_advanced(text, adjusted_confidence, linguistic_features)
        ...
        return {
            ...
            'sentiment': sentiment_result if not isinstance(sentiment_result, Exception) else {'label': 'UNKNOWN', 'score': 0.5},
            'conversation_context': context_emotion,
            'linguistic_features': linguistic_features,
            'analysis_method': 'heavy_fusion',
            'emotion_sources_used': len(emotion_sources),
            'api_success_rate': len([r for r in results if not isinstance(r, Exception)]) / len(results),
  ...
  "reasoning": "brief explanation"
    ...
    async def _call_embedding_emotion_analysis(self, text: str) -> Optional[Dict]:
        ...
        try:
            ...
            if total_score > 0:
                normalized_emotions = {e: score/total_score for e, score in emotion_scores.items()}
            else:
    ...
    def _analyze_with_keywords(self, text: str) -> Dict[str, Any]:
        ...
        emotion_patterns = {
            'joy': {
                'strong': ['ecstatic', 'thrilled', 'overjoyed', 'elated', 'fantastic', 'amazing', 'incredible'],
                'medium': ['happy', 'excited', 'delighted', 'cheerful', 'joyful', 'wonderful', 'great'],
    ...
    def _fuse_emotion_results(self, emotion_sources: List[tuple]) -> Dict:
        ...
        for source_name, result in emotion_sources:
            ...
            emotions = result.get('all_emotions', {result.get('primary_emotion', 'neutral'): result.get('confidence', 0.5)})
        ...
        primary_emotion = max(all_emotions.items(), key=lambda x: x[1])[0] if all_emotions else 'neutral'
    ...
    def _calculate_emotional_intensity(self, text: str, base_confidence: float) -> float:
        ...
        intensity_factors = {
            ...
            'capital_letters': min(sum(1 for c in text if c.isupper()) / len(text), 0.2) if len(text) > 0 else 0,
        ...
        intensifiers = ['very', 'really', 'extremely', 'incredibly', 'absolutely', 'totally', 'completely', 'so', 'super']
    ...
    def _calculate_emotional_intensity_advanced(self, text: str, base_confidence: float, linguistic_features: Dict) -> float:
    ...
    def _analyze_linguistic_features(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_emotional_words': any(word in text.lower() for word in ['feel', 'emotion', 'heart', 'soul', 'mind']),
            'average_word_length': sum(len(word) for word in text.split()) / len(text.split()) if text.split() else 0
        }
    
    async def _analyze_conversation_context_light(self, user_id: str, conversation_history: List[str]) -> Optional[Dict]:
    ...
    async def _analyze_conversation_context_heavy(self, user_id: str, conversation_history: List[str]) -> Optional[Dict]:
        """Heavy conversation context analysis (multiple API calls)"""
        
        light_context = await self._analyze_conversation_context_light(user_id, conversation_history)
        ...
        try:
            ...
            emotion_tasks = [self._call_lm_studio_emotion(msg) for msg in sample_messages[:3]]  # Limit API calls
                ...
                async with self.session.get(f"{self.llm_api_url}/v1/models") as response:
                    if response.status == 200:
                        ...
                        if self.logger:
                            ...
                            self.logger.debug(f"🔧 LLM provider: HTTP {response.status} (using external embeddings as primary)")
        except Exception as e:
            if self.logger:
                # Downgrade to debug since it's optional when using external embeddings
                ...
                if self.session:
                    async with self.session.get(
                        ...
                        if response.status == 200:
                            ...
                            if self.logger:
                                self.logger.debug(f"🔧 Hugging Face: HTTP {response.status} (using external embeddings as primary)")
            except Exception as e:
                if self.logger:
                    self.logger.debug(f"🔧 Hugging Face: {e} (using external embeddings as primary)")
        else:
            if self.logger:
                # Remove warning since Hugging Face is now optional (we use external embeddings)
                self.logger.debug("🔧 Hugging Face: No API key provided (using external embeddings as primary)")
    
    def build_cloud_emotional_prompt(self, emotion_analysis: Dict, phase3_context: Optional[Dict] = None) -> str:
