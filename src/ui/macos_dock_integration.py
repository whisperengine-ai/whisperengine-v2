    ...
    def _update_dock_icon(self):
        ...
        try:
            ...
            if self.status.server_running:
                if self.status.active_connections > 0:
                    # Active state
                    self._set_dock_progress(self.status.active_connections / 10.0)  # Max 10 connections
                else:
                    # Idle state
    ...
    def _on_new_activity(self):
        ...
        try:
            if self.preferences.get("show_activity_notifications", True):
                self._notify("New Activity", f"New conversation started ({self.status.active_connections} active)")
