...
class EnhancedMacOSTray:
            ...
            return Menu(
                ...
                MenuItem(
                    ...
                    Menu(
                        ...
                        MenuItem(
                            f"Connections: {self.status.active_connections}", None, enabled=False
                        ),
                        MenuItem(
                            f"Conversations Today: {self.status.conversations_today}",
                            None,
                            enabled=False,
                        ),
                ...
                MenuItem(
                    ...
                    Menu(
                        MenuItem(
                            ...
                            checked=lambda item: self.preferences.get(
                                "show_activity_notifications", True
                            ),
                        ),
                        MenuItem(
                            "🏷️ Badge Count",
                            self._toggle_badge_count,
                            checked=lambda item: self.preferences.get("show_badge_count", True),
                        ),
                        MenuItem(
                            "💾 Memory Monitor",
                            self._toggle_memory_monitor,
    ...
    def _create_status_icon(self, badge_count: Optional[int] = None):
        ...
        try:
            ...
            if badge_count and badge_count > 0 and self.preferences.get("show_badge_count", True):
    ...
    def _update_tray_icon(self):
        ...
        try:
            badge_count = (
                ...
                if self.preferences.get("show_badge_count", True)
                else None
            )
    ...
    def _show_recent_conversations(self, icon=None, item=None):
        ...
        self._notify(
            "Recent Conversations", "Feature coming soon - will show recent conversation history"
        )
...
def create_enhanced_macos_tray(
    ...
    if not TRAY_AVAILABLE:
        logging.getLogger(__name__).warning(
            "Enhanced tray not available - pystray/PIL not installed"
        )
        return None
