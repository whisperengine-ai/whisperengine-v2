...
class NativeSettingsManager(QObject):
    ...
    def _load_from_qt_settings(self):
        ...
        try:
            # LLM settings
            self.llm_config.api_url = str(
                self.qt_settings.value("llm/api_url", self.llm_config.api_url)
            )
            self.llm_config.model_name = str(
                self.qt_settings.value("llm/model_name", self.llm_config.model_name)
            )
            ...
            try:
                ...
                self.llm_config.temperature = (
                    float(str(temp_val)) if temp_val is not None else self.llm_config.temperature
                )
            ...
            try:
                ...
                self.ui_config.font_size = (
                    int(str(font_size_val))
                    if font_size_val is not None
                    else self.ui_config.font_size
                )
            except (ValueError, TypeError):
            ...
            try:
                ...
                self.platform_config.system_tray_enabled = (
                    str(tray_val).lower() == "true" if tray_val is not None else True
                )
            except (ValueError, TypeError):
    ...
    def _save_to_qt_settings(self):
        ...
        try:
            ...
            self.qt_settings.setValue(
                "platform/system_tray", self.platform_config.system_tray_enabled
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.llm_config.model_name in model_names:
                    return (
                        True,
                        f"✅ Connected successfully in {response_time:.2f}s. Model '{self.llm_config.model_name}' is available.",
                    )
                else:
                    available_models = ", ".join(model_names[:3])
                    return (
                        False,
                        f"⚠️ Connected but model '{self.llm_config.model_name}' not found. Available: {available_models}",
                    )
            else:
