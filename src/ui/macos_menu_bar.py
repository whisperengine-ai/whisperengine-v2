...
class WhisperEngineMacOSApp:
    ...
    def _create_icon_image(self):
        ...
        # Outer circle
        draw.ellipse(
            [8, 8, 56, 56], fill=(100, 149, 237, 255), outline=(70, 130, 180, 255), width=2
        )
    ...
    def _create_tray_icon(self):
        ...
        try:
                ...
                pystray.MenuItem(
                    ...
                    pystray.Menu(
                        ...
                        pystray.MenuItem(
                            ...
                            checked=lambda item: self.preferences.get("auto_open_browser", True),
                        ),
                        pystray.MenuItem(
                            "Show Notifications",
                            self.toggle_notifications,
                            checked=lambda item: self.preferences.get("show_notifications", True),
                        ),
                    ),
                ),
                pystray.Menu.SEPARATOR,
            ...
            self.tray_icon = pystray.Icon(
                "WhisperEngine", icon_image, "WhisperEngine - AI Conversation Platform", menu=menu
            )
        except Exception as e:
    ...
    def show_preferences(self, icon=None, item=None):
        ...
        try:
            ...
            set prefs to display dialog "WhisperEngine Preferences" & return & return & ¬
                "Auto-open browser: " & "'''
                + ("Yes" if self.preferences.get("auto_open_browser", True) else "No")
                + '''" & return & ¬
                "Show notifications: " & "'''
                + ("Yes" if self.preferences.get("show_notifications", True) else "No")
                + """" ¬
    ...
    def toggle_notifications(self, icon=None, item=None):
        ...
        self.preferences["show_notifications"] = not self.preferences.get(
            "show_notifications", True
        )
        self._save_preferences()
...
def create_macos_menu_bar(
    ...
    if not is_macos_menu_available():
        logging.getLogger(__name__).warning(
            "macOS menu bar not available - pystray/PIL not installed or not on macOS"
        )
        return None
    ...
    def clear_history(self, _):
        ...
        if response == 1:  # OK clicked
            try:
                ...
                self.logger.info("History cleared by user")
                rumps.notification(
                    "WhisperEngine", "History Cleared", "All conversations have been cleared."
                )
            except Exception as e:
                self.logger.error(f"Failed to clear history: {e}")
...
© 2025 WhisperEngine AI"""
    ...
    def restart_server(self, _):
        ...
        try:
            ...
            rumps.notification(
                "WhisperEngine", "Server Restarting", "The server is being restarted..."
            )
            self.logger.info("Server restart requested")
        except Exception as e:
            self.logger.error(f"Failed to restart server: {e}")
...
class PreferencesWindow:
    ...
    def show(self):
        ...
        rumps.notification(
            "WhisperEngine", "Preferences Saved", "Your preferences have been updated."
        )
