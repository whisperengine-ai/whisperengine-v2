...
class GraphPersonalityManager:
    ...
    async def store_personality_profile(self, user_id: str, metrics: PersonalityMetrics, 
        ...
        try:
            async with self.driver.session() as session:
                ...
                if hasattr(personality_data.get('communication_style'), 'value'):
                    personality_data['communication_style'] = personality_data['communication_style'].value
                if hasattr(personality_data.get('directness_style'), 'value'):
                    personality_data['directness_style'] = personality_data['directness_style'].value
                ...
                await session.execute_write(self._create_personality_profile, 
                                          user_id, personality_data, conversation_context)
    ...
    @staticmethod
    async def _create_personality_profile(tx, user_id: str, personality_data: Dict, 
    ...
    async def analyze_and_store_personality(self, user_id: str, messages: List[str], 
    ...
    def _generate_recommendations(self, personality: Dict) -> List[str]:
        ...
        if emotional_expr > 0.7:
            recommendations.append("Comfortable with emotional discussions and empathetic responses")
    ...
    async def update_personality_from_conversation(self, user_id: str, new_messages: List[str]) -> bool:
        """
    ...
    def _merge_personality_metrics(self, existing: Dict, new: PersonalityMetrics) -> PersonalityMetrics:
        ...
        merged.openness = existing.get('openness', 0.5) * existing_weight + new.openness * new_weight
        merged.conscientiousness = existing.get('conscientiousness', 0.5) * existing_weight + new.conscientiousness * new_weight
        merged.extraversion = existing.get('extraversion', 0.5) * existing_weight + new.extraversion * new_weight
        merged.agreeableness = existing.get('agreeableness', 0.5) * existing_weight + new.agreeableness * new_weight
        merged.neuroticism = existing.get('neuroticism', 0.5) * existing_weight + new.neuroticism * new_weight
        
        # Merge behavioral metrics
        merged.avg_message_length = existing.get('avg_message_length', 0.0) * existing_weight + new.avg_message_length * new_weight
        merged.complexity_score = existing.get('complexity_score', 0.5) * existing_weight + new.complexity_score * new_weight
        merged.emotional_expressiveness = existing.get('emotional_expressiveness', 0.5) * existing_weight + new.emotional_expressiveness * new_weight
        merged.confidence_level = existing.get('confidence_level', 0.5) * existing_weight + new.confidence_level * new_weight
        merged.detail_orientation = existing.get('detail_orientation', 0.5) * existing_weight + new.detail_orientation * new_weight
        merged.abstract_thinking = existing.get('abstract_thinking', 0.5) * existing_weight + new.abstract_thinking * new_weight
        merged.social_engagement = existing.get('social_engagement', 0.5) * existing_weight + new.social_engagement * new_weight
        ...
        # Update metadata
        merged.total_messages_analyzed = existing.get('total_messages_analyzed', 0) + new.total_messages_analyzed
