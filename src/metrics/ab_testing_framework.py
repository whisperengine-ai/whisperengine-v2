...
class ABTestingFramework:
    ...
    def __init__(
        self, metrics_collector: HolisticAIMetrics, redis_client=None, database_manager=None
    ):
    ...
    async def record_test_interaction(
        self, user_id: str, test_type: TestType, metrics: ConversationMetrics
    ):
    ...
    async def analyze_test_results(self, test_id: str) -> dict[str, TestResult]:
        ...
        for variant in test.variants:
            ...
            for metrics_dict in metrics_list:
                ...
                cns = await self.metrics_collector.calculate_conversation_naturalness_score(
                    metrics_obj
                )
            ...
            avg_emotion = statistics.mean(
                [m.get("emotional_appropriateness", 0) for m in metrics_list]
            )
            avg_response_time = statistics.mean(
                [m.get("total_response_time", 0) for m in metrics_list]
            )
    ...
    async def get_test_recommendations(self, test_id: str) -> dict[str, Any]:
        ...
        best_variant = max(results.values(), key=lambda r: r.avg_conversation_naturalness)
        ...
        if control_result and best_variant != control_result:
            improvement = (
                (
                    best_variant.avg_conversation_naturalness
                    - control_result.avg_conversation_naturalness
                )
                / control_result.avg_conversation_naturalness
                * 100
            )
        ...
        recommendation = {
            ...
            "ready_for_deployment": (
                best_variant.statistical_significance > 0.95
                and best_variant.conversation_count
                >= self.active_tests[test_id].minimum_sample_size
                and improvement > 5  # At least 5% improvement
            ),
        }
