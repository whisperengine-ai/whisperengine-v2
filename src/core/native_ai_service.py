...
class NativeAIService:
        ...
        try:
            ...
            if self.universal_chat:
                ...
                conversation_context = self.universal_chat.build_conversation_context(
                    ...
                    conversation_id or "native_chat", 
                    message
                )
                
                ai_response = await self.universal_chat.generate_ai_response(message_obj, conversation_context)
    ...
    def process_message(self, message: str, conversation_id: Optional[str] = None) -> AIMessage:
        ...
        if not self.event_loop:
            return AIMessage(
                content="AI service not started",
                timestamp=datetime.now(),
                message_type="error"
            )
        ...
        future = asyncio.run_coroutine_threadsafe(
            self.process_message_async(message, conversation_id),
            self.event_loop
        )
