...
class MessageProcessor:
    ...
    async def _process_hybrid_query_routing(
        self,
        message_context: MessageContext,
        conversation_context,  # Can be List[Dict] or Dict[str, Any]
        ai_components: Dict[str, Any]
    ) -> Optional[List[Dict[str, Any]]]:
        """
        Phase 6.9: Hybrid Query Routing - Assess complexity and route to LLM tools if needed.
        
        This method:
        1. Assesses query complexity using HybridQueryRouter
        2. Routes to LLM tool calling if complexity threshold met
        3. Executes tools via ToolExecutor
        4. Returns tool results for context enrichment
        
        Args:
            message_context: Current message context
            conversation_context: Built conversation context
            ai_components: AI intelligence components
        
        Returns:
            List of tool results if tool calling used, None otherwise
        """
        try:
            ...
            # Initialize HybridQueryRouter (could be cached in __init__ later)
            router = HybridQueryRouter(
                complexity_threshold=0.3,  # TODO: Make configurable
            ...
            should_use_tools, assessment = router.should_use_tools(
                user_message=message_context.content,
                user_id=message_context.user_id,
                character_name=character_name,
            ...
            if not should_use_tools:
                logger.debug(
                    "🔍 HYBRID ROUTING: Using semantic search | Score: %.3f | Threshold: %.3f",
                    assessment.complexity_score,
                    router.complexity_threshold
                )
                return None
            
            # Complexity threshold met - use LLM tool calling
            logger.info(
                "🔧 HYBRID ROUTING: Using tool calling | Score: %.3f | Reasoning: %s",
                assessment.complexity_score,
            ...
            postgres_pool = getattr(self.bot_core, 'postgres_pool', None) if self.bot_core else None
            vector_memory = self.memory_manager if self.memory_manager else None
            influxdb_client = getattr(self, 'temporal_client', None)
            ...
            # Call LLM with tools to determine which tool(s) to use
            # Build a simple prompt for tool selection
            tool_selection_messages = [
                {
                    "role": "system",
                    "content": (
                        f"You are {character_name}, an AI assistant with access to tools "
                        f"to retrieve information. Analyze the user's question and determine "
                        f"which tool(s) would help you answer it accurately."
                    )
                },
                {
                    "role": "user",
            ...
            # Use LLM client's tool calling capability
            try:
                tool_response = await self.llm_client.generate_chat_completion_with_tools(
                ...
                tool_calls = getattr(tool_response, 'tool_calls', None)
                if not tool_calls:
                    logger.info("🔧 HYBRID ROUTING: LLM decided not to use tools")
                    return None
                ...
                logger.info(
                    "🔧 HYBRID ROUTING: Executed %d tools | Success: %d | Failures: %d",
                    len(tool_results),
                ...
                # LLM client doesn't have generate_chat_completion_with_tools
                logger.warning(
                    "🔧 HYBRID ROUTING: LLM client does not support tool calling - "
                    "falling back to semantic search"
                )
                return None
            
        except ImportError as e:
            logger.warning("🔧 HYBRID ROUTING: Not available (import error): %s", e)
            return None
        except Exception as e:
            logger.error("🔧 HYBRID ROUTING: Error during tool routing: %s", e, exc_info=True)
            return None
    
    async def _enrich_context_with_tool_results(
        self,
        conversation_context,  # Can be List[Dict] or Dict[str, Any]
        tool_results: List[Dict[str, Any]]
    ):
        """
        Enrich conversation context with tool results for LLM consumption.
        
        Formats tool results and adds them to the conversation context so the LLM
        can use the retrieved information to generate a more accurate response.
        
        Args:
            conversation_context: Existing conversation context (List or Dict)
            tool_results: List of tool execution results
        
        Returns:
            Enriched conversation context with tool results
        """
        if not tool_results:
            return conversation_context
        
        # Format tool results for LLM consumption
        tool_results_text = "\n\n## Tool Results (Retrieved Information)\n\n"
        ...
        for result in tool_results:
            ...
            tool_results_text += f"### {tool_name}\n"
            
            if success and data:
                if isinstance(data, list):
                    tool_results_text += f"Retrieved {len(data)} items:\n"
                    # Show first few items
                    for item in data[:5]:  # Limit to first 5 items
                        tool_results_text += f"- {json.dumps(item, indent=2)}\n"
                    if len(data) > 5:
                        tool_results_text += f"... and {len(data) - 5} more items\n"
                elif isinstance(data, dict):
                    tool_results_text += f"{json.dumps(data, indent=2)}\n"
                else:
                    tool_results_text += f"{data}\n"
            elif error:
                tool_results_text += f"Error: {error}\n"
            else:
                tool_results_text += "No data retrieved\n"
            
            tool_results_text += "\n"
        
        # Handle both List[Dict] (messages format) and Dict (structured format)
        if isinstance(conversation_context, list):
            # Messages format - append tool results as a system message
            conversation_context.append({
                "role": "system",
                "content": tool_results_text
            })
        elif isinstance(conversation_context, dict):
            # Dict format - add to additional_context
            if "additional_context" in conversation_context:
                conversation_context["additional_context"] += "\n" + tool_results_text
            else:
                conversation_context["additional_context"] = tool_results_text
        
        logger.debug(
            "🔧 CONTEXT ENRICHMENT: Added %d tool results (%d chars) to conversation context",
            len(tool_results),
