...
class Phase2Integration:
    ...
    async def process_message_with_emotional_intelligence(
        ...
        try:
            ...
            if (
                assessment.recommended_intervention
                and self.auto_intervention_enabled
                and self._should_execute_intervention(user_id)
            ):
            ...
            logger.debug(
                f"Emotional intelligence processing complete: {assessment.phase_status.value} status"
            )
    ...
    def _generate_response_guidance(
        ...
        if stress.stress_level.value in ["high", "critical"]:
            ...
            guidance["avoid_topics"].extend(
                ["additional_complexity", "overwhelming_information", "non_urgent_topics"]
            )
    ...
    async def handle_intervention_response(
        ...
        try:
            ...
            if outcome.effectiveness_score >= 0.8:
                logger.info(
                    f"Highly effective intervention {intervention_id} - learning from success"
                )
    ...
    async def get_user_emotional_status(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            dashboard["integration_status"] = {
                ...
                "last_intervention": (
                    self.last_interventions.get(user_id, {}).isoformat()
                    if user_id in self.last_interventions
                    else None
                ),
    ...
    async def generate_proactive_check_in(self, user_id: str) -> str | None:
        ...
        try:
            ...
            if should_check_in and self._should_execute_intervention(user_id):
                ...
                logger.info(
                    f"Generated proactive check-in for {user_id}: {', '.join(check_in_reason)}"
                )
                return message
    ...
    def configure_auto_intervention(self, enabled: bool, cooldown_hours: float | None = None):
        ...
        logger.info(
            f"Auto-intervention configured: enabled={enabled}, cooldown={self.intervention_cooldown}"
        )
    ...
    async def force_assessment(
        ...
        try:
            ...
            return {"forced_assessment": False, "error": str(e), "user_id": user_id}
...
def initialize_phase2_integration(
    bot_instance=None, graph_personality_manager=None, conversation_cache=None
):
    """Initialize global Phase 2 integration instance"""
    global phase2_integration
    phase2_integration = Phase2Integration(
        bot_instance, graph_personality_manager, conversation_cache
    )
