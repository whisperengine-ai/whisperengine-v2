...
class ProactiveSupport:
    ...
    def __init__(self):
        ...
        self._effectiveness_indicators = {
            'positive_response': [
                'thank you', 'helpful', 'appreciate', 'better', 'good idea',
                'makes sense', 'that works', 'feeling better', 'glad you asked'
            ],
            'neutral_response': [
                'okay', 'sure', 'maybe', 'i guess', 'possibly', 'alright'
            ],
            'negative_response': [
                'not helpful', 'doesn\'t work', 'not what i need', 'prefer not to',
                'too much', 'overwhelming', 'not now', 'leave me alone'
            ],
            'crisis_indicators': [
                'give up', 'no point', 'can\'t take it', 'want to disappear',
                'hopeless', 'worthless', 'nobody cares', 'end it all'
            ]
        }

    async def analyze_support_needs(self, user_id: str, 
        ...
        if stress.get('coping_indicators', []):
            effective_coping = [
                indicator for indicator in stress['coping_indicators'] 
        ...
        if user_history:
            ...
            recurring_stressors = self._identify_recurring_patterns(recent_interactions, 'stressors')
        ...
        logger.debug(f"Support needs analysis complete: urgency level {analysis['support_urgency']}")
        return analysis

    def _identify_recurring_patterns(self, interactions: List[Dict], pattern_type: str) -> List[str]:
        ...
        if pattern_type == 'stressors':
            ...
            patterns = [
                category for category, count in stressor_mentions.items() 
                if count >= 3
            ]
    ...
    def _identify_effective_strategies(self, user_history: List[Dict]) -> List[str]:
        ...
        for i in range(len(user_history) - 1):
            ...
            if ('intervention' in current and 
                'mood_assessment' in current and 
                'mood_assessment' in next_interaction):
    ...
    async def _generate_intervention_recommendations(self, needs_analysis: Dict, 
    ...
    async def create_support_intervention(self, user_id: str,
                                        support_needs: Dict[str, Any],
                                        user_strategy: Optional[SupportStrategy] = None) -> SupportIntervention:
        """
        ...
        logger.debug(f"Created intervention {intervention.intervention_id} with priority {intervention.priority_level}")
        return intervention
    ...
    async def _generate_personalized_message(self, support_type: SupportType,
                                           intervention_style: InterventionStyle,
                                           user_strategy: Optional[SupportStrategy],
                                           support_needs: Dict) -> str:
        ...
        if '{strategy}' in base_message:
            effective_approaches = personalization_insights.get('effective_approaches', ['breathing exercises'])
        ...
        if '{positive_pattern}' in base_message:
            base_message = base_message.replace('{positive_pattern}', 'managing challenging situations')
    ...
    def _determine_expected_outcome(self, support_type: SupportType, 
                                  intervention_style: InterventionStyle) -> str:
        """Determine expected outcome for intervention"""
        outcomes = {
            (SupportType.CRISIS, InterventionStyle.CRISIS_INTERVENTION): "immediate emotional stabilization and safety confirmation",
            (SupportType.REACTIVE, InterventionStyle.DIRECT_SUPPORT): "stress reduction and problem-solving engagement",
            (SupportType.PREVENTIVE, InterventionStyle.GENTLE_CHECK_IN): "early issue identification and rapport maintenance",
            (SupportType.MAINTENANCE, InterventionStyle.SKILL_BUILDING): "skill reinforcement and confidence building"
        }
        
        return outcomes.get((support_type, intervention_style), "positive user engagement and emotional support")
    ...
    async def deliver_intervention(self, intervention: SupportIntervention,
    ...
    async def _validate_delivery_timing(self, intervention: SupportIntervention,
                                      delivery_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if recent_interventions:
            last_intervention_time = max(recent_interventions, key=lambda x: x.get('timestamp', datetime.min))
            time_since_last = datetime.now(timezone.utc) - last_intervention_time.get('timestamp', datetime.min)
        ...
        if len(today_interventions) >= self.max_daily_interventions and intervention.priority_level < 5:
            return {
                'can_deliver': False,
    ...
    async def track_intervention_outcome(self, intervention_id: str,
                                       user_response: str,
                                       outcome_context: Dict[str, Any]) -> SupportOutcome:
        """
        ...
        follow_up_needed = (
            effectiveness_score < 0.4 or 
            crisis_matches > 0 or
            outcome_type in ["ineffective", "crisis_escalation"]
        )
        ...
        logger.debug(f"Intervention outcome: {outcome_type} (effectiveness: {effectiveness_score:.2f})")
        return outcome

    def generate_support_summary(self, intervention: SupportIntervention,
                                outcome: Optional[SupportOutcome] = None) -> Dict[str, Any]:
