...
async def stream_to_tempfile(chunk_generator: AsyncGenerator[bytes, None], logger: Optional[logging.Logger] = None) -> str:
    """
    ...
    try:
        ...
        temp_file.close()
        logger.debug(f"Streaming complete: {chunks_received} chunks, {total_bytes} bytes -> {temp_path}")
    
    return temp_path


async def create_streaming_audio_source(chunk_generator: AsyncGenerator[bytes, None], logger: Optional[logging.Logger] = None) -> tuple[discord.FFmpegPCMAudio, str]:
    """
