...
class DiscordVoiceManager:
    ...
    def __init__(self, bot: commands.Bot, elevenlabs_client: ElevenLabsClient, 
        ...
        self.voice_listening_enabled = os.getenv("VOICE_LISTENING_ENABLED", "true").lower() == "true"
        self.voice_streaming_enabled = os.getenv("VOICE_STREAMING_ENABLED", "true").lower() == "true"
        ...
        self.logger.debug(f"Voice settings - Auto-join: {self.auto_join_enabled}, Response: {self.voice_response_enabled}, Listening: {self.voice_listening_enabled}")
    ...
    async def join_voice_channel(self, channel: discord.VoiceChannel) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Joined voice channel '{channel.name}' in guild '{channel.guild.name}'")
    ...
    async def start_listening(self, guild_id: int):
        ...
        if not voice_state.voice_client or not voice_state.voice_client.is_connected():
            self.logger.error(f"Cannot start listening: not connected to voice channel in guild {guild_id}")
            return
    ...
    async def _listen_for_voice(self, guild_id: int):
        ...
        self.logger.info(f"🎤 Voice listening enabled for guild {guild_id} - with connection keepalive")
        ...
        if voice_join_announcements:
            ...
            if current_channel:
                text_channels = [ch for ch in current_channel.guild.text_channels 
    ...
    async def _voice_connection_keepalive(self, guild_id: int):
        ...
        # Configuration from environment
        keepalive_interval = float(os.getenv("VOICE_KEEPALIVE_INTERVAL", "300"))  # Default 5 minutes
        heartbeat_interval = float(os.getenv("VOICE_HEARTBEAT_INTERVAL", "30"))   # Default 30 seconds
                        ...
                        if reconnect_attempts < max_reconnect_attempts:
                            ...
                            if current_channel:
                                try:
                                    ...
                                    self.logger.info(f"Successfully reconnected to voice channel in guild {guild_id}")
                            ...
                            self.logger.error(f"Max reconnection attempts reached for guild {guild_id}")
                            break
    ...
    async def _send_keepalive_audio(self, guild_id: int):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as temp_file:
                ...
                try:
                    subprocess.run([
                        'ffmpeg', '-f', 's16le', '-ar', '44100', '-ac', '2',
                        '-i', temp_pcm, '-y', temp_file.name
    ...
    async def _queue_audio_for_processing(self, guild_id: int, user_id: int, audio_file: io.BytesIO):
        ...
        try:
            ...
            estimated_duration = len(audio_data) / (self.sample_rate * self.sample_width * self.channels)
    ...
    async def _generate_voice_response(self, guild_id: int, user_id: int, text: str, display_name: str):
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": f"You are responding via voice chat to {display_name}. Keep responses conversational and under 100 words."},
                {"role": "user", "content": text}
            ]
            
            # Get LLM response
            response = await asyncio.wait_for(
                self.llm_client.get_chat_response(messages),
                timeout=10.0
            )
    ...
    async def _process_tts_queue(self, guild_id: int):
        ...
        try:
            while not voice_state.tts_queue.empty():
                try:
                    tts_item = await asyncio.wait_for(
                        voice_state.tts_queue.get(),
                        timeout=1.0
                    )
    ...
    async def _speak_text(self, guild_id: int, text: str):
        ...
        try:
            ...
            use_streaming = (self.voice_streaming_enabled and 
                           getattr(self.elevenlabs, 'use_streaming', True) and
                           hasattr(self.elevenlabs, 'text_to_speech_stream'))
            ...
            if use_streaming:
                ...
                try:
                    ...
                    audio_source, temp_filename = await create_streaming_audio_source(chunk_generator, self.logger)
    ...
    def is_connected(self, guild_id: int) -> bool:
        ...
        voice_state = self.get_voice_state(guild_id)
        return (voice_state.voice_client is not None and 
                voice_state.voice_client.is_connected())
