...
class FollowUpHandler:
    ...
    async def execute(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            if not channel_id:
                error_msg = f"Missing required payload data: user_id={user_id}, channel_id={channel_id}"
                logger.error(error_msg)
                raise ValueError(error_msg)
            ...
            if self.bot_client:
                try:
                    ...
                    if channel:
                        ...
                        logger.info(f"Sent follow-up message to user {user_id} in channel {channel_id}")
                    else:
