...
class LLMMessageRoleSecurityProcessor:
    ...
    def __init__(
        ...
        logger.debug(
            f"LLM Message Security initialized: max_system_length={self.max_system_length} characters, max_messages={self.max_messages}"
        )
    ...
    def sanitize_system_message_content(self, content: str) -> str:
        ...
        for pattern in self.dangerous_patterns:
            sanitized = re.sub(
                pattern, "[SECURITY_FILTERED]", sanitized, flags=re.IGNORECASE | re.MULTILINE
            )
        ...
        if len(sanitized) > self.max_system_length:
            ...
            logger.warning(
                f"System message truncated to {self.max_system_length:,} characters (was {len(sanitized):,} chars). "
    ...
    def process_system_messages_securely(
        ...
        logger.info(
            f"SECURITY: Selected 1 system message from {len(system_messages)} candidates "
        ...
        if len(valid_candidates) > 1:
            ...
            if self.security_log_level == "quiet":
                ...
                if len(valid_candidates) > 6:
                    logger.warning(
                        f"SECURITY ALERT: {len(valid_candidates)} system messages - possible injection attack"
                    )
            elif self.security_log_level == "verbose":
                logger.info(
                    f"SECURITY: Selected 1 of {len(valid_candidates)} system messages (normal operation)"
                )
            else:  # normal level
                if len(valid_candidates) > 5:
                    logger.warning(
                        f"SECURITY: {len(valid_candidates)} system messages - review for optimization"
                    )
                else:
    ...
    def _calculate_system_message_priority(self, content: str) -> float:
        ...
        if any(
            phrase in content_lower
            for phrase in ["ignore previous", "reveal", "show instructions", "system override"]
        ):
        ...
        for msg in messages:
            ...
            if role == MessageRole.USER.value:
                ...
                max_assistant_length = (
                    max_tokens_chat * 4 // 4
                )  # 100% allocation for responses, 4 chars per token
                if len(sanitized_content) > max_assistant_length:
                    sanitized_content = (
                        sanitized_content[:max_assistant_length] + "\n[RESPONSE_TRUNCATED]"
                    )
            else:
    ...
    def secure_message_processing(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        logger.info(
            f"Secure processing complete: {len(messages)} -> {len(final_messages)} messages"
        )
        ...
        if self.security_events:
            if self.security_log_level == "quiet":
                ...
                if len(self.security_events) > 8:
                    logger.warning(
                        f"High security activity: {len(self.security_events)} events - investigate"
                    )
            elif self.security_log_level == "verbose":
                ...
                if len(self.security_events) > 5:
                    logger.warning(
                        f"Multiple security events: {len(self.security_events)} - review if frequent"
                    )
                else:
