...
class SecureDebugLogger:
    ...
    def sanitize_sensitive_data(self, data: str) -> str:
        ...
        sensitive_patterns = [
            ...
            (
                ...
                r"\1[PASSWORD_MASKED]",
            ),  # Password with capture group
            (
                r"((?:token|key|secret)[\s:=]+)\S+",
                r"\1[TOKEN_MASKED]",
            ),  # API keys/tokens with capture group
        ]
    ...
    def log_user_debug_info(
        ...
        hashed_message = (
            f"msg_{hashlib.sha256(message_id.encode()).hexdigest()[:8]}" if message_id else "no_msg"
        )
...
def secure_add_debug_info_to_response(
    response: str,
    user_id: str,
    memory_manager,
