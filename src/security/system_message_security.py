...
class SystemMessageSecurityFilter:
    ...
    def _load_configuration(self) -> Dict[str, Any]:
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                relative_path = os.path.relpath(self.config_path)
                logger.warning(
                    f"Configuration file not found: {relative_path}, using fallback patterns"
                )
                return {}
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            ...
            config_path = str(
                project_root / "config" / "security" / "response_leakage_patterns.json"
            )
        ...
        self.compiled_patterns = [
            re.compile(pattern, re.IGNORECASE | re.DOTALL) for pattern in self.SENSITIVE_PATTERNS
        ]
    ...
    def sanitize_system_messages(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for message in messages:
            ...
            if sanitized_content and sanitized_content.strip():
                ...
                logger.debug(
                    f"Sanitized system message: {len(content)} -> {len(sanitized_content)} chars"
                )
            else:
    ...
    def _sanitize_system_content(self, content: str) -> str:
        ...
        for pattern in self.compiled_patterns:
            if pattern.search(sanitized):
                logger.warning(
                    f"Removing sensitive pattern from system message: {pattern.pattern[:50]}..."
                )
                sanitized = pattern.sub("[SENSITIVE_CONTENT_REMOVED]", sanitized)

        # Clean up multiple consecutive markers
        sanitized = re.sub(
            r"\[SENSITIVE_CONTENT_REMOVED\]\s*\[SENSITIVE_CONTENT_REMOVED\]",
    ...
    def scan_response_for_leakage(self, response: str) -> Dict[str, Any]:
        ...
        for i, pattern in enumerate(self.compiled_patterns):
            ...
            if matches:
                ...
                sanitized_response = pattern.sub(
                    "[SYSTEM_INFORMATION_FILTERED]", sanitized_response
                )
        ...
        for indicator in leakage_indicators:
            if re.search(indicator, response, re.IGNORECASE):
                ...
                sanitized_response = re.sub(
                    indicator, "[CONTEXT_FILTERED]", sanitized_response, flags=re.IGNORECASE
                )
    ...
    def create_secure_system_message(
        ...
        if message_type == "character":
            ...
            safe_content = (
                "You are a helpful AI assistant with a thoughtful and caring personality."
            )
    ...
    def log_system_message_security_event(
