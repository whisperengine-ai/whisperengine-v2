...
class AsyncContextBoundariesManager:
    ...
    async def update_user_preferences(
        ...
        updated_prefs = PrivacyPreferences(
            ...
            allow_cross_server=(
                allow_cross_server
                if allow_cross_server is not None
                else current_prefs.allow_cross_server
            ),
            allow_dm_to_server=(
                allow_dm_to_server
                if allow_dm_to_server is not None
                else current_prefs.allow_dm_to_server
            ),
            allow_server_to_dm=(
                allow_server_to_dm
                if allow_server_to_dm is not None
                else current_prefs.allow_server_to_dm
            ),
            allow_private_to_public=(
                allow_private_to_public
                if allow_private_to_public is not None
                else current_prefs.allow_private_to_public
            ),
    ...
    def _determine_context_boundary_type(
        self, source_context: MemoryContext, target_context: MemoryContext
    ) -> str:
        ...
        if source_type == MemoryContextType.DM and target_type in [
            MemoryContextType.PUBLIC_CHANNEL,
            MemoryContextType.PRIVATE_CHANNEL,
        ]:
            return "dm_to_server"

        # Server to DM boundaries
        if (
            source_type in [MemoryContextType.PUBLIC_CHANNEL, MemoryContextType.PRIVATE_CHANNEL]
            and target_type == MemoryContextType.DM
        ):
            return "server_to_dm"

        # Cross-server boundaries
        if (
            source_context.server_id
            and target_context.server_id
            and source_context.server_id != target_context.server_id
        ):
            return "cross_server"

        # Private to public boundaries
        if (
            source_type == MemoryContextType.PRIVATE_CHANNEL
            and target_type == MemoryContextType.PUBLIC_CHANNEL
        ):
            return "private_to_public"

        # Public to private boundaries
        if (
            source_type == MemoryContextType.PUBLIC_CHANNEL
            and target_type == MemoryContextType.PRIVATE_CHANNEL
        ):
    ...
    async def check_context_boundary(
        self, user_id: str, source_context: MemoryContext, target_context: MemoryContext
    ) -> tuple[bool, str]:
        """
        ...
        if boundary_type == "dm_to_server" and not preferences.allow_dm_to_server:
            ...
            await self._log_decision(
                user_id,
                self._get_context_type_string(source_context),
                self._get_context_type_string(target_context),
                "blocked",
                reason,
        ...
        if boundary_type in default_rules and not default_rules[boundary_type]:
            reason = (
                f"Default privacy level {preferences.privacy_level.value} rule: {boundary_type}"
            )
            await self._log_decision(
                user_id,
                self._get_context_type_string(source_context),
