...
class CostOptimizationEngine:
    ...
    def _calculate_confidence(
        self, user_id: str, model_name: str, context: RequestContext
    ) -> float:
        ...
        if (
            context.conversation_type in ["technical", "analysis"]
            and "reasoning" in model_profile.strengths
        ):
            base_confidence += 0.1
    ...
    def _apply_selection_logic(
        ...
        for pred in predictions:
            ...
            speed_score = 1 - (
                pred.estimated_response_time_ms
                / max(p.estimated_response_time_ms for p in predictions)
            )

            # Weighted composite score
            composite_score = cost_score * 0.4 + confidence_score * 0.4 + speed_score * 0.2
    ...
    async def get_cost_optimization_suggestions(self, user_id: str) -> List[str]:
        ...
        try:
            ...
            if not suggestions:
                suggestions.append(
                    "Your usage patterns look efficient! Consider setting monthly budget alerts."
                )
    ...
    async def check_budget_status(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "days_remaining": (
                    datetime.now().replace(month=datetime.now().month + 1, day=1) - datetime.now()
                ).days,
            }
