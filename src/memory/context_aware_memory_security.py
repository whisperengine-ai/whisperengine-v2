...
class MemoryContext:
    ...
    def __post_init__(self):
        ...
        if (
            ...
            and self.context_type != MemoryContextType.DM
        ):
...
class ContextAwareMemoryManager:
    ...
    def __init__(self, base_memory_manager):
        ...
        self.context_compatibility = {
            ...
            ContextSecurity.PRIVATE_CHANNEL: [
                ContextSecurity.PRIVATE_CHANNEL,
                ContextSecurity.CROSS_SERVER,
            ],
            ContextSecurity.PUBLIC_CHANNEL: [
                ContextSecurity.PUBLIC_CHANNEL,
                ContextSecurity.CROSS_SERVER,
            ],
    ...
    def get_compatible_contexts(self, current_context: MemoryContext) -> list[ContextSecurity]:
        ...
        return self.context_compatibility.get(
            current_context.security_level, [current_context.security_level]
        )
    ...
    def retrieve_context_aware_memories(
        ...
        try:
            ...
            logger.debug(
                f"Context filtering: {len(all_memories)} -> {len(filtered_memories)} memories"
            )
    ...
    def _is_context_compatible(
        ...
        if (
            current_context.server_id
            and memory_context.server_id
            and current_context.server_id == memory_context.server_id
        ):
            return True

        # DM context check
        if (
            current_context.context_type == MemoryContextType.DM
            and memory_context.context_type == MemoryContextType.DM
        ):
            return True
        ...
        **kwargs,
        ...
        try:
            ...
            if context:
                ...
                logger.debug(
                    f"Storing conversation with context: {context.context_type.value} (security: {context.security_level.value})"
                )
