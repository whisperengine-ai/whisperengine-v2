...
class AdvancedTopicClusterer:
    ...
    async def classify_memory_topic(self, memory_id: str, content: str, 
                                  existing_topics: Optional[List[str]] = None) -> MemoryTopicInfo:
        """
        ...
        best_cluster, confidence = await self._find_best_cluster_match(embedding, keywords, existing_topics)
        
        # Get secondary topic matches
        secondary_topics = await self._find_secondary_topics(embedding, best_cluster, existing_topics)
    ...
    async def get_topic_recommendations(self, content: str, limit: int = 5) -> List[Tuple[str, float]]:
        """
    ...
    async def create_new_topic_cluster(self, name: str, initial_memories: List[str], 
                                     description: Optional[str] = None) -> TopicCluster:
        """
        ...
        top_keywords = [kw for kw, count in keyword_counts.most_common(self.keyword_extraction_limit)]
        
        # Generate description if not provided
        if not description:
            description = await self._generate_cluster_description(name, top_keywords, initial_memories)
        ...
        self.logger.info("Created new topic cluster: %s with %d memories", name, len(initial_memories))
        return cluster
    ...
    async def recluster_all_topics(self, force: bool = False) -> Dict[str, Any]:
        ...
        new_clusters = await self._create_clusters_from_labels(memory_ids, embeddings, cluster_labels)
    ...
    async def find_similar_clusters(self, cluster_id: str, limit: int = 5) -> List[Tuple[str, float]]:
        """
    ...
    async def merge_clusters(self, cluster_id1: str, cluster_id2: str, new_name: Optional[str] = None) -> Optional[str]:
        """
        ...
        embeddings1 = [np.array(self.memory_topic_info[mid].embedding_vector) 
                      for mid in cluster1.memory_ids 
                      if mid in self.memory_topic_info and self.memory_topic_info[mid].embedding_vector]
        embeddings2 = [np.array(self.memory_topic_info[mid].embedding_vector) 
        ...
        keyword_counts = Counter(all_keywords)
        merged_keywords = [kw for kw, count in keyword_counts.most_common(self.keyword_extraction_limit)]
    ...
    async def _llm_extract_keywords(self, content: str) -> List[str]:
        ...
        messages = [
            {"role": "system", "content": "You are a keyword extraction expert. Return only valid JSON."},
    ...
    async def _find_best_cluster_match(self, embedding: np.ndarray, keywords: List[str], 
                                     existing_topics: Optional[List[str]] = None) -> Tuple[str, float]:
    ...
    async def _calculate_cluster_match_score(self, embedding: np.ndarray, keywords: List[str], 
                                           cluster: TopicCluster) -> float:
    ...
    async def _find_secondary_topics(self, embedding: np.ndarray, primary_cluster: str, 
                                   existing_topics: Optional[List[str]] = None) -> List[str]:
    ...
    async def _create_new_cluster_from_content(self, embedding: np.ndarray, keywords: List[str]) -> str:
        ...
        description = f"Cluster focused on {', '.join(keywords[:3])}" if keywords else "New topic cluster"
    ...
    async def _update_cluster_membership(self, memory_id: str, cluster_id: str, 
                                       embedding: np.ndarray, keywords: List[str]):
        ...
        keyword_counts = Counter(all_keywords)
        cluster.keywords = [kw for kw, count in keyword_counts.most_common(self.keyword_extraction_limit)]
    ...
    async def _adaptive_clustering(self, embedding_matrix: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            kmeans_score = silhouette_score(embedding_matrix, kmeans_labels) if len(set(kmeans_labels)) > 1 else -1
            dbscan_score = silhouette_score(embedding_matrix, dbscan_labels) if len(set(dbscan_labels)) > 1 else -1
    ...
    async def _create_clusters_from_labels(self, memory_ids: List[str], embeddings: List[np.ndarray], 
                                         cluster_labels: np.ndarray) -> Dict[str, TopicCluster]:
    ...
    async def _update_memory_cluster_assignments(self, memory_ids: List[str], cluster_labels: np.ndarray):
    ...
    async def _generate_cluster_description(self, name: str, keywords: List[str], memory_ids: List[str]) -> str:
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": "You are an expert at creating concise, informative descriptions."},
                {"role": "user", "content": prompt}
            ]
    ...
    def get_clustering_stats(self) -> Dict[str, Any]:
        ...
        clustered_memories = sum(1 for info in self.memory_topic_info.values() 
                               if info.primary_topic != "uncategorized")
