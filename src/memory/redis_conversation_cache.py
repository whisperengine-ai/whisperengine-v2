...
class RedisConversationCache:
    ...
    def __init__(self, cache_timeout_minutes=15, bootstrap_limit=20, max_local_messages=50):
        ...
        logger.info(f"RedisConversationCache initialized: timeout={cache_timeout_minutes}min, "
    ...
    def _serialize_message(self, message: discord.Message) -> bytes:
        ...
        message_data = {
            ...
            'mentions': [str(user.id) for user in message.mentions] if message.mentions else []  # Convert to strings
        }
    ...
    async def get_conversation_context(self, channel, limit=5, exclude_message_id=None) -> List[Dict[str, Any]]:
        """
        ...
        if not messages:
            logger.warning(f"No cached messages found in Redis for channel {channel_id} after bootstrap - falling back to Discord history")
            try:
    ...
    async def get_user_conversation_context(self, channel, user_id: int, limit=5, exclude_message_id=None) -> List[Dict[str, Any]]:
        """
        ...
        return user_relevant_messages[-limit:] if len(user_relevant_messages) >= limit else user_relevant_messages
            ...
            for i, serialized_msg in enumerate(messages):
                try:
                    ...
                    if msg_data['id'] == str(message_id):
                        ...
                        logger.debug(f"Removed message {message_id} from Redis cache in channel {channel_id}")
                        break
                except Exception as e:
    ...
    async def sync_with_storage(self, channel_id: str, message: discord.Message, storage_success: bool):
    ...
    async def _needs_bootstrap(self, channel_id: str, requested_limit: int, current_time: float) -> bool:
    ...
    async def _get_cached_messages(self, channel_id: str, limit: int, exclude_message_id: Optional[int]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            serialized_messages = await self.redis.lrange(message_key, 0, limit * 2)  # Get extra in case we need to filter
