...
class GraphEnhancedMemoryManager(UserMemoryManager):
    ...
    async def generate_personalized_system_prompt(self, user_id: str, current_message: str) -> str:
        ...
        try:
            ...
            if triggers:
                sensitive_topics = [t["topic"] for t in triggers if t.get("avg_intensity", 0) > 0.7]
                if sensitive_topics:
                    prompt_elements.append(
    ...
    async def update_relationship_milestone(
        self, user_id: str, milestone_level: str, context: str = ""
    ):
        ...
        try:
            ...
            await graph_connector.execute_write_query(
                query, {"user_id": user_id, "level": milestone_level, "context": context}
            )
    ...
    async def analyze_conversation_for_milestones(
        ...
        if any(phrase in message_lower for phrase in ["my name is", "i'm ", "call me"]):
            await self.update_relationship_milestone(
                user_id, "first_name", "User shared their name"
            )
            return "first_name"
        ...
        if any(indicator in message_lower for indicator in personal_indicators):
            await self.update_relationship_milestone(
                user_id, "shared_personal", "User shared personal information"
            )
        ...
        if any(indicator in message_lower for indicator in trust_indicators):
            await self.update_relationship_milestone(
                user_id, "trusted_with_secret", "User shared confidential information"
            )
    ...
    async def get_graph_health_status(self) -> Dict[str, Any]:
        ...
        graph_connector = await self._get_graph_connector()
        if not graph_connector:
            return {"status": "disabled", "message": "Graph database is disabled or unavailable"}
    ...
    async def store_conversation_async(self, user_id: str, message: str, response: str):
        ...
        memory_id = await self.store_conversation_enhanced(user_id, message, response, emotion_data)
