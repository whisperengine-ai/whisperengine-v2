...
class DesktopConversationCache:
        ...
        try:
            ...
            with self.lock:
                ...
                if len(self.memory_cache[conversation_id]) > self.max_messages_per_conversation:
                    self.memory_cache[conversation_id] = self.memory_cache[conversation_id][
    ...
    def clear_channel_cache(self, channel_id: str):
        ...
        try:
            ...
            with self.lock:
                # Clear from memory
                keys_to_remove = [
                    key for key in self.memory_cache.keys() if key.startswith(conversation_pattern)
                ]
    ...
    async def _cleanup_database(self):
        ...
        try:
            ...
            logger.debug(
                f"Database cleanup: {deleted_messages} messages, {deleted_conversations} conversations removed"
            )
