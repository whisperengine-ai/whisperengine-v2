...
class ChromaDBHTTPManager:
    ...
    def __init__(self, host: Optional[str] = None, port: Optional[int] = None):
        ...
        if self.use_external_embeddings:
            logger.info("External embeddings detected - ChromaDB will not load local embedding models")
    ...
    async def _ensure_collections(self):
        ...
        try:
            ...
            try:
                self.global_collection = self.client.get_collection(name=self.global_collection_name)
                logger.info(f"Connected to existing global collection: {self.global_collection_name}")
            except Exception:
    ...
    async def store_conversation(self, user_id: str, message: str, response: str, metadata: Optional[Dict] = None):
    ...
    async def store_user_fact(self, user_id: str, fact: str, category: Optional[str] = None, confidence: float = 1.0, metadata: Optional[Dict] = None):
    ...
    async def store_global_fact(self, fact: str, category: Optional[str] = None, confidence: float = 1.0, metadata: Optional[Dict] = None):
    ...
    async def search_memories(self, query_text: str, user_id: Optional[str] = None, limit: int = 5, 
        ...
        try:
            ...
            if self.global_collection and (not doc_types or "global_fact" in doc_types):
                ...
                if global_results and global_results["documents"] and global_results["documents"][0]:
    ...
    async def get_user_facts(self, user_id: str, category: Optional[str] = None, limit: int = 20) -> List[Dict]:
...
async def store_conversation_async(user_id: str, message: str, response: str, metadata: Dict = None) -> str:
...
async def store_user_fact_async(user_id: str, fact: str, category: str = None, confidence: float = 1.0, metadata: Dict = None) -> str:
    """Store user fact (async)"""
    manager = await get_chromadb_manager()
    return await manager.store_user_fact(user_id, fact, category, confidence, metadata)

async def search_memories_async(query_text: str, user_id: str = None, limit: int = 5, doc_types: List[str] = None) -> List[Dict]:
