...
class SemanticDeduplicator:
    ...
    def register_memory(
        self, memory_id: str, content: str, memory_type: str = "conversation"
    ) -> MemoryFingerprint:
        """
        ...
        try:
            ...
            for memory_id, fingerprint in self.memory_fingerprints.items():
                try:
                    ...
                    if (
                        preview_similarity < min_similarity - 0.1
                    ):  # Allow some margin for full comparison
                        continue
                    ...
                    self.logger.debug(
                        "Error computing similarity for memory %s: %s", memory_id, str(e)
                    )
                    continue
    ...
    def optimize_memory_storage(self, memory_ids: list[str]) -> dict[str, Any]:
        ...
        if total_groups > 0:
            avg_group_size = (
                sum(len(group["memories"]) for group in optimization_report["duplicate_groups"])
                / total_groups
            )
    ...
    def _quick_similarity_check(
        self, query_embedding: np.ndarray, fingerprint: MemoryFingerprint
    ) -> float:
        ...
        return self._quick_similarity_check_previews(
            query_preview.tolist(), fingerprint.embedding_preview
        )
    ...
    def _compute_fingerprint_similarity(
        self, fp1: MemoryFingerprint, fp2: MemoryFingerprint
    ) -> float:
