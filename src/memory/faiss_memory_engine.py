...
class FaissMemoryIndex:
    ...
    def __init__(
        self, dimension: int = 384, index_type: str = "IVF", nlist: int = 100, use_gpu: bool = False
    ):
        """
    ...
    def _create_index(self):
        ...
        if self.use_gpu and hasattr(faiss, "StandardGpuResources"):
            logger.warning(
                "GPU acceleration disabled - local processing uses CPU-only for optimal compatibility"
            )
    ...
    def search(
        ...
        with self._lock:
            ...
            search_k = (
                min(k * 3, self.index.ntotal)
                if (user_filter or doc_type_filter or min_importance > 0.0)
                else k
            )
    ...
    def batch_search(
        ...
        with self._lock:
            ...
            logger.debug(
                f"🔍 Faiss batch search: {len(query_vectors)} queries in {search_time*1000:.2f}ms"
            )
    ...
    async def _batch_processor(self):
        ...
        while True:
            try:
                ...
                should_process = len(batch) >= 10 or (  # Batch size threshold
                ...
                if should_process and batch:
                    ...
                    if fact_docs:
                        await asyncio.get_event_loop().run_in_executor(
                            self.thread_pool, self.fact_index.add_documents, fact_docs
                        )
...
class FaissChromaDBAdapter:
    ...
    async def search_memories_with_embedding(
        ...
        results = await self.engine.search_memories(
            query_embedding=query_embedding, user_id=user_id, k=limit, doc_types=doc_types
        )
