...
class AdvancedConversationSummarizer:
    ...
    async def should_summarize_conversation(self, user_id: str, messages: List[Dict[str, Any]]) -> bool:
        """
    ...
    async def create_conversation_summary(self, user_id: str, messages: List[Dict[str, Any]]) -> ConversationSummary:
        """
        ...
        self.logger.info("Creating conversation summary for user %s with %d messages", user_id, len(messages))
        ...
        self.logger.info("Created summary %s with %.2f%% compression ratio", summary_id, compression_ratio * 100)
        return summary
    ...
    async def _analyze_conversation_structure(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _extract_key_topics(self, messages: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            ...
            messages_for_llm = [
                {"role": "system", "content": "You are a topic extraction expert. Return only valid JSON."},
    ...
    def _extract_keywords_fallback(self, messages: List[Dict[str, Any]]) -> List[str]:
        ...
        common_words = {
            ...
            'them', 'well', 'were', 'what', 'your', 'about', 'could', 'there', 'think'
        }
    ...
    async def _extract_important_facts(self, messages: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            ...
            if self.memory_manager and hasattr(self.memory_manager, 'fact_extractor'):
                for msg in messages:
                    if msg.get('role') == 'user' and msg.get('content'):
                        extracted = self.memory_manager.fact_extractor.extract_global_facts_from_message(
                            msg['content']
                        )
    ...
    async def _analyze_emotional_evolution(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if emotion_counts:
                emotional_context['dominant_emotion'] = max(emotion_counts.items(), key=lambda x: x[1])[0]
    ...
    async def _create_conversation_flow(self, messages: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        for msg in messages:
            ...
            if role == 'user':
                ...
                current_exchange['bot_response'] = content[:100] + '...' if len(content) > 100 else content
    ...
    async def _generate_context_tags(self, messages: List[Dict[str, Any]], 
                                   topics: List[str], facts: List[str]) -> List[str]:
    ...
    async def _generate_natural_summary(self, messages: List[Dict[str, Any]], 
                                      topics: List[str], facts: List[str],
                                      emotional_context: Dict[str, Any],
                                      conversation_flow: List[Dict[str, str]]) -> str:
    ...
    def _generate_fallback_summary(self, messages: List[Dict[str, Any]], 
                                 topics: List[str], facts: List[str]) -> str:
    ...
    def _cache_summary(self, summary: ConversationSummary):
        ...
        self.summary_cache[summary.summary_id] = {
            'summary': summary,
        ...
        to_remove = [
            sid for sid, data in self.summary_cache.items()
    ...
    async def summarize_if_needed(self, user_id: str, messages: List[Dict[str, Any]]) -> Optional[ConversationSummary]:
        """
