...
class UserMemoryManager:
    def __init__(
        ...
        enable_auto_facts: bool = None,
        enable_global_facts: bool = None,
        enable_emotions: bool = None,
        llm_client=None,
    ):
        ...
        try:
            ...
            if enable_emotions:
                self.emotion_manager = EmotionManager(
                    llm_client=self.llm_client, memory_manager=self
                )
                logger.info(
                    "Emotion and relationship management enabled with LLM-based sentiment analysis"
                )
            else:
            ...
            if GRAPH_MEMORY_AVAILABLE and enable_global_facts:
                ...
                logger.info(
                    "Graph memory manager disabled (Neo4j not available or global facts disabled)"
                )
            ...
            try:
                ...
                logger.info(
                    "UserMemoryManager initialized with ChromaDB (user and global collections)"
                )
            except Exception as e:
    ...
    def _validate_user_id(self, user_id: str) -> str:
        ...
        if (
            user_id.startswith("test_")
            or "test" in user_id.lower()  # Allow any test-related user IDs
            or user_id.isdigit()
            or user_id == "desktop_admin"
            or self._is_valid_uuid(user_id)
        ):
            return user_id
        else:
    ...
    def store_conversation(
        ...
        bot_response: str,
        channel_id: str = None,
        pre_analyzed_emotion_data: dict = None,
        metadata: dict = None,
    ):
        ...
        try:
            ...
            if self.enable_emotions and self.emotion_manager:
                try:
                    ...
                    if pre_analyzed_emotion_data:
                        ...
                        user_profile, emotion_profile = self.emotion_manager.process_interaction(
                        ...
                        logger.debug(
                            f"Analyzed emotion during storage for user {user_id}: {emotion_profile.detected_emotion.value}"
                        )
                    ...
                    if emotion_data:
                        logger.debug(
                            f"Emotion processed for user {user_id}: {emotion_data.get('detected_emotion', 'unknown')} "
            ...
            logger.debug(
                f"Successfully stored conversation for user {user_id}: {user_message[:50]}..."
            )
            logger.debug(
                f"Conversation storage - User message length: {len(user_message)}, Response length: {len(bot_response)}"
            )
            ...
            try:
                ...
                self._extract_and_store_personality_facts(
                    user_id, user_message, bot_response, timestamp, metadata or {}
                )
    ...
    def _extract_and_store_personality_facts(
        self, user_id: str, user_message: str, bot_response: str, timestamp: str, metadata: dict
    ):
        ...
        try:
            ...
            if facts_stored > 0:
                logger.debug(
                    f"Automatically stored {facts_stored} personality facts for user {user_id}"
                )
            else:
    ...
    def get_related_global_facts(
        ...
        try:
            ...
            if (
                ...
                and include_graph_relationships
                and getattr(self, "_graph_memory_manager_initialized", False)
                and self.graph_memory_manager
            ):
                        ...
                        if "id" in fact_dict:
                            ...
                            related_facts = asyncio.run(
                                self.graph_memory_manager.get_related_facts(
                                    chromadb_id=chromadb_id, max_depth=2
                                )
                            )
        ...
        try:
            graph_facts = asyncio.run(
                self.graph_memory_manager.search_facts_by_domain(
                    knowledge_domain=domain, include_subdomain=True, limit=limit
                )
            )
    ...
    def get_all_global_facts(self, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            for i, doc in enumerate(results["documents"]):
                ...
                facts.append({"id": fact_id, "content": doc, "metadata": metadata})
    ...
    def store_personality_fact(self, user_id: str, fact_content: str, context_metadata=None):
        ...
        try:
            ...
            doc_id = (
                f"{user_id}_personality_{datetime.now().isoformat()}_{hash(fact_content) % 10000}"
            )
            ...
            logger.debug(
                ...
                user_id,
                personality_fact.fact_type.value,
    ...
    def retrieve_personality_facts(
        self,
        user_id: str,
        query: str = "",
    ...
    def retrieve_relevant_memories(self, user_id: str, query: str, limit: int = 10) -> List[Dict]:
        ...
        try:
                ...
                memories.append(
                    {
                        ...
                        "relevance_score": global_fact["relevance_score"]
                        + 0.1,  # Boost global fact scores
                    }
                )
            ...
            logger.debug(
                f"Retrieved {len(memories)} relevant memories for user {user_id} (including {len(global_facts)} global facts)"
            )
            return memories
                ...
                if metadata.get("type") == "user_fact":
                    ...
                    if "user_message" in metadata:
                        summary_parts.append(
                            f"• Recently discussed: {metadata['user_message'][:100]}..."
                        )
                ...
                for memory in memories:
                    ...
                    if metadata.get("type") == "user_fact":
                        ...
                        context_parts.append(
                            f"• Previously discussed: {metadata['user_message'][:100]}..."
                        )
