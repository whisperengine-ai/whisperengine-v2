...
try:
    from src.personality.memory_moments import (
        MemoryTriggeredMoments, MemoryConnection, ConversationContext,
        MemoryMomentType, MemoryConnectionType
    )
...
class EnhancedMemorySystem:
    ...
    async def get_memory_analytics_enhanced(self) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics['basic_stats'] = {
                ...
                'avg_importance': np.mean([node.importance_score for node in self.memory_nodes.values()]) if self.memory_nodes else 0,
                'total_accesses': sum(node.access_count for node in self.memory_nodes.values())
            }
            
            # Performance statistics
            avg_search_time = (
                self.search_stats['total_search_time_ms'] / max(self.search_stats['total_searches'], 1)
            )
            analytics['performance'] = {
                'avg_search_time_ms': avg_search_time,
                'faiss_searches': self.search_stats['faiss_searches'],
                'fallback_searches': self.search_stats['fallback_searches'],
                'search_efficiency': 'ultra_fast' if avg_search_time < 10 else 'fast' if avg_search_time < 50 else 'normal'
            }
            ...
            if self.memory_graph and len(self.memory_graph.nodes) > 0:
                analytics['network_analysis'] = {
                    ...
                    'avg_connections_per_memory': self.memory_graph.number_of_edges() / max(self.memory_graph.number_of_nodes(), 1),
            ...
            if self.memory_df is not None and len(self.memory_df) > 0:
                analytics['temporal_analysis'] = {
                    'memories_per_day': len(self.memory_df) / max((datetime.now() - self.memory_df['timestamp'].min()).days, 1),
                    'most_active_memory_type': self.memory_df['memory_type'].mode().iloc[0] if len(self.memory_df) > 0 else "unknown",
    ...
    def _create_empty_search_result(self, start_time: datetime, algorithm: str) -> MemorySearchResult:
