...
class MemoryImportanceEngine:
    ...
    async def calculate_memory_importance(self, memory_id: str, user_id: str, 
                                        memory_data: Dict, user_history: List[Dict],
                                        memory_manager=None) -> MemoryImportanceScore:
        """
        ...
        try:
            ...
            personal_relevance = await self._assess_personal_relevance(memory_data, user_id, user_history)
    ...
    async def _assess_personal_relevance(self, memory_data: Dict, user_id: str, 
        ...
        personal_indicators = [
            'my', 'mine', 'myself', 'i am', 'i was', 'i have', 'i had', 'i feel',
            'i think', 'i believe', 'i want', 'i need', 'my family', 'my job',
            'my life', 'my experience', 'my problem', 'my goal', 'personal'
        ]
        ...
        topic_frequency = sum(1 for interaction in user_history 
        ...
        disclosure_keywords = [
            'secret', 'private', 'personal', 'confess', 'admit', 'share', 'tell you',
            'between us', 'don\'t tell', 'trust you', 'opening up'
        ]
        ...
        goal_keywords = [
            'goal', 'dream', 'hope', 'plan', 'future', 'want to', 'aspire',
            'vision', 'ambition', 'achieve', 'succeed'
        ]
    ...
    async def _get_access_frequency(self, memory_id: str, user_id: str, memory_manager=None) -> float:
    ...
    async def _assess_uniqueness(self, memory_data: Dict, user_id: str, 
                               user_history: List[Dict]) -> float:
        ...
        similar_topics = sum(1 for interaction in user_history 
                           if topic and topic in interaction.get('topic', '').lower())
    ...
    async def _check_milestone_significance(self, memory_data: Dict, user_history: List[Dict]) -> float:
        ...
        relationship_milestones = [
            'first time', 'anniversary', 'birthday', 'graduation', 'promotion',
            'wedding', 'engagement', 'birth', 'death', 'funeral', 'celebration',
            'achievement', 'award', 'success', 'failure', 'breakthrough',
            'milestone', 'turning point', 'life-changing', 'important moment'
        ]
    ...
    def _generate_score_explanation(self, factor_scores: ImportanceFactors, overall_score: float) -> str:
    ...
    async def _apply_time_decay(self, memory_id: str, user_id: str, memory_data: Dict, memory_manager):
    ...
    async def _boost_recently_connected_memories(self, user_id: str, user_memories: List[Dict], memory_manager):
    ...
    async def _adjust_for_emotional_patterns(self, user_id: str, user_memories: List[Dict], memory_manager):
    ...
    async def identify_core_memories(self, user_id: str, limit: Optional[int] = None, memory_manager=None) -> List[Dict]:
        """
    ...
    def _create_default_importance_score(self, memory_id: str, user_id: str) -> MemoryImportanceScore:
    ...
    def get_importance_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        user_scores = [
            score for key, score in self.importance_cache.items() 
    ...
    def _calculate_factor_averages(self, user_scores: List[MemoryImportanceScore]) -> Dict[str, float]:
        ...
        return {
            factor: statistics.mean(values) if values else 0.0 
    ...
    def clear_cache(self, user_id: Optional[str] = None):
        ...
        if user_id:
            ...
            keys_to_remove = [key for key in self.importance_cache.keys() if key.startswith(f"{user_id}_")]
