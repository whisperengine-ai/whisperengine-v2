...
class AdvancedMemoryBatcher:
    ...
    def __init__(self, 
        ...
        logger.info(f"🚀 Initialized AdvancedMemoryBatcher with batch_size={batch_size}, "
    ...
    async def store_conversation_batch(self, user_id: str, message: str, response: str,
    ...
    async def store_user_fact_batch(self, user_id: str, fact: str,
                                  metadata: Optional[Dict[str, Any]] = None,
                                  priority: int = 0) -> str:
    ...
    async def query_memories_batch(self, user_id: str, query: str, limit: int = 10,
    ...
    async def _process_batch(self, operation_type: str, batch: List[BatchOperation]):
        ...
        try:
            ...
            logger.debug(f"Processed {len(batch)} {operation_type} operations in {processing_time*1000:.1f}ms")
    ...
    async def _process_conversation_batch(self, conversations: List[BatchOperation]):
        ...
        df['memory_id'] = [f"conv_{hash(f'{row.user_id}_{row.message}_{row.timestamp}')%1000000}" 
        ...
        for _, row in df.iterrows():
            try:
                ...
                memory_id = await asyncio.get_event_loop().run_in_executor(
                    ...
                    row['user_id'], row['message'], row['response'], row['metadata']
                )
    ...
    def _store_single_conversation(self, user_id: str, message: str, response: str, 
        ...
        for operation in user_facts:
            try:
                ...
                fact_id = await asyncio.get_event_loop().run_in_executor(
                    ...
                    operation.user_id, operation.data['fact'], operation.data['metadata']
                )
    ...
    def _execute_single_query(self, user_id: str, query: str, limit: int, 
                            doc_types: List[str]) -> List[Dict[str, Any]]:
    ...
    def _is_duplicate_operation(self, operation_hash: str) -> bool:
        ...
        self.recent_operations = {
            h: ts for h, ts in self.recent_operations.items()
    ...
    def _generate_query_cache_key(self, user_id: str, query: str, limit: int,
                                doc_types: Optional[List[str]]) -> str:
...
class BatchedMemoryAdapter:
    ...
    async def store_conversation(self, user_id: str, message: str, response: str,
        ...
        if self.batcher:
            return await self.batcher.store_conversation_batch(
                user_id, message, response, metadata
            )
        else:
            # Direct call
            return self.chromadb_manager.store_conversation(user_id, message, response, metadata or {})
