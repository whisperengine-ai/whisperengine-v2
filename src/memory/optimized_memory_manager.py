...
class OptimizedMemoryManager(IntegratedMemoryManager):
        ...
        try:
            ...
            if self.enable_clustering and self.embedding_manager:
                self.topic_clusterer = AdvancedTopicClusterer(
                    embedding_manager=self.embedding_manager, llm_client=self.llm_client
                )
    ...
    def store_conversation(
        ...
        if any([self.enable_summarization, self.enable_deduplication, self.enable_clustering]):
            asyncio.create_task(
                self._optimize_user_memory_background(user_id, user_message, bot_response)
            )

        return result

    async def _optimize_user_memory_background(
        ...
        try:
            ...
            if self.semantic_deduplicator:
                ...
                memory_ids = [
                    mem.get("memory_id", mem.get("id", "")) for mem in user_memories[-20:]
                ]  # Last 20 memories
                ...
                if memory_ids:
                    ...
                    if dedup_result.get("optimization_opportunities", []):
                        logger.debug(
                            f"Found {len(dedup_result['optimization_opportunities'])} optimization opportunities for user {user_id}"
                        )
    ...
    def retrieve_relevant_memories(self, user_id: str, query: str, limit: int = 10):
        ...
        base_memories = super().retrieve_relevant_memories(
            user_id, query, limit * 2
        )  # Get more for prioritization
        ...
        if self.context_prioritizer and base_memories:
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"Prioritized {len(prioritized_memories)} memories for user {user_id}"
                    )
    ...
    async def _optimize_single_user_memory(self, user_id: str):
        ...
        logger.info(
            f"Running memory optimization for user {user_id} ({len(user_memories)} memories)"
        )
        ...
        if self.semantic_deduplicator:
            ...
            if dedup_result.get("duplicate_groups", []):
                logger.info(
                    f"Found {len(dedup_result['duplicate_groups'])} duplicate groups for user {user_id}"
                )

        # 3. Context gap analysis
        if self.context_prioritizer:
            gap_analysis = await self.context_prioritizer.analyze_context_gaps(
                user_id, "recent activity"
            )

            if gap_analysis["coverage_score"] < 0.7:
                logger.info(
                    f"User {user_id} has low context coverage: {gap_analysis['coverage_score']:.2f}"
                )
    ...
    async def _get_recent_conversation_history(self, user_id: str, limit: int = 20) -> list[dict]:
        ...
        try:
            ...
            for memory in recent_memories[-limit:]:
                ...
                role = (
                    "user"
                    if "User:" in content or not content.startswith(("I ", "Here", "Based on"))
                    else "assistant"
                )
...
def create_optimized_memory_manager(
    llm_client=None, embedding_manager=None, memory_manager=None
...
async def run_periodic_memory_optimization(
    memory_manager: OptimizedMemoryManager, interval_hours: int = 24
):
    """
