...
class LocalVectorStorage:
    ...
    def __init__(self, storage_dir: Optional[Path] = None, embedding_dim: int = 384):
        ...
        self.documents: Dict[str, Dict[str, VectorDocument]] = {}  # collection_name -> doc_id -> document
        ...
        logger.info(f"LocalVectorStorage initialized: dim={embedding_dim}, storage={self.storage_dir}")
    ...
    def add_documents(self, collection_name: str, documents: List[str], 
        ...
        with self._lock:
            ...
            for i, (doc, embedding, meta) in enumerate(zip(documents, embeddings, metadata)):
                ...
                doc_id = ids[i] if ids and i < len(ids) else self._generate_doc_id(doc, meta.get('user_id'))
    ...
    def get_documents(self, collection_name: str, ids: Optional[List[str]] = None,
                     where: Optional[Dict[str, Any]] = None, limit: Optional[int] = None,
                     offset: Optional[int] = None) -> Dict[str, List[Any]]:
...
class LocalVectorStorageAdapter:
    ...
    def _ensure_default_collections(self):
        """Ensure default collections exist"""
        for collection_name in [self.conversations_collection, self.user_facts_collection, self.global_facts_collection]:
    ...
    async def store_conversation(self, user_id: str, message: str, response: str, 
                               embeddings: List[float], metadata: Optional[Dict[str, Any]] = None) -> str:
    ...
    async def search_memories(self, query_embeddings: List[float], user_id: Optional[str] = None, 
                            limit: int = 10, doc_types: Optional[List[str]] = None) -> List[Dict[str, Any]]:
