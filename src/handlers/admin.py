...
class AdminCommandHandlers:
    ...
    def __init__(self, bot, llm_client, memory_manager, backup_manager, conversation_history, 
                 health_monitor, job_scheduler, context_memory_manager, phase2_integration):
        self.bot = bot
    ...
    async def _schedule_followup_handler(self, ctx, user_mention, hours, is_admin):
        ...
        try:
            ...
            await ctx.send(f"✅ Follow-up message scheduled for <@{user_id}> in {hours} hours.\nJob ID: `{job_id}`")
            logger.info(f"Admin {ctx.author.name} scheduled follow-up for user {user_id} in {hours} hours")
    ...
    async def _job_status_handler(self, ctx, job_id, is_admin):
        ...
        try:
            if job_id:
                ...
                embed = discord.Embed(title="Job Scheduler Status", color=0x00ff00 if stats['running'] else 0xff0000)
                embed.add_field(name="Status", value="🟢 Running" if stats['running'] else "🔴 Stopped", inline=True)
                ...
                if recent:
                    embed.add_field(name="24h Executions", value=str(recent.get('total_executions', 0)), inline=True)
                    embed.add_field(name="24h Success Rate", 
    ...
    async def _followup_settings_handler(self, ctx, setting):
        ...
        try:
            ...
            if setting.lower() == "off" or setting.lower() == "disable":
                ...
                await ctx.send("✅ Follow-up messages disabled. You won't receive automatic follow-ups.")
                ...
                await ctx.send("✅ Follow-up messages enabled! I'll check in with you if you're away for a while. This is an experimental feature - thanks for testing!")
                ...
                embed = discord.Embed(title="Follow-up Settings", color=0x00ff00 if enabled else 0xff0000)
                embed.add_field(name="Status", value=f"Follow-ups are **{status}**", inline=False)
                if not enabled:
                    embed.add_field(name="🧪 Experimental Feature", 
                                   value="Follow-up messages are an opt-in experimental feature. Use `!followup_settings on` to enable.", 
                                   inline=False)
                embed.add_field(name="Usage", value="`!followup_settings [on|off|status]`", inline=False)
                embed.add_field(name="What are follow-ups?", 
    ...
    async def _health_check_handler(self, ctx, is_admin):
        ...
        try:
            ...
            embed.add_field(
                name="Uptime", 
                value=self.health_monitor.get_uptime(),
                inline=True
            )
            embed.add_field(
                name="Last Check", 
    ...
    async def _memory_stats_handler(self, ctx, is_admin):
        ...
        try:
            ...
            embed.add_field(name="User Memories", value=stats.get('total_memories', 'Unknown'), inline=True)
            embed.add_field(name="Global Facts", value=stats.get('total_global_facts', 'Unknown'), inline=True)
            ...
            embed.add_field(name="User Collection", value=stats.get('user_collection_name', 'Unknown'), inline=True)
            embed.add_field(name="Global Collection", value=stats.get('global_collection_name', 'Unknown'), inline=True)
            embed.add_field(name="Avg Messages/Channel", value=f"{conv_stats['avg_messages_per_channel']:.1f}", inline=True)
    ...
    async def _system_status_handler(self, ctx, is_admin):
        ...
        try:
            ...
            llm_status = f"✅ Connected{service_info}" if connection_ok else f"❌ Disconnected{service_info}"
            ...
            embed.add_field(name="💬 Active Conversations", value=f"✅ {conv_stats['channels']} channels, {conv_stats['total_messages']} messages", inline=False)
            embed.add_field(name="💾 Backup System", value=backup_status, inline=False)
            
            # Add bot info
            embed.add_field(name="📊 Bot Info", value=f"✅ Connected to {len(self.bot.guilds)} servers", inline=False)
    ...
    async def _emotional_intelligence_handler(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(
                title="🎯 Predictive Emotional Intelligence Status",
                color=0x9b59b6
            )
            ...
            if self.phase2_integration:
                ...
                try:
                    test_result = await self.phase2_integration.process_message_with_emotional_intelligence(
                        user_id=user_id,
                        message="Hello, this is a status check",
                        conversation_context={"source": "status_command", "context": "emotional_intelligence_status"}
                    )
