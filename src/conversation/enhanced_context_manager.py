...
class EnhancedConversationContextManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"EnhancedConversationContextManager initialized with boundary management: {enable_boundary_management}")
    ...
    async def get_enhanced_conversation_context(self, 
        ...
        if self.enable_boundary_management and self.boundary_manager:
            ...
            context['context_metadata'] = await self._build_context_metadata(session, boundary_context)
            
            # Add conversation guidance
            context['conversation_guidance'] = await self._generate_conversation_guidance(session, message_content)
    ...
    async def handle_multi_user_context(self, 
                                       channel, 
        ...
        context['target_user_messages'] = await self._format_messages_for_context(target_messages[-limit_per_user:])
        
        # Analyze other users' activity
        other_users = [uid for uid in user_messages.keys() if uid != target_user_id and not any(msg.author.bot for msg in user_messages[uid])]
        ...
        for other_user_id in other_users[:5]:  # Limit to 5 other users
            ...
            context['other_user_activity'][str(other_user_id)] = {
                ...
                'last_activity': other_messages[-1].created_at.isoformat() if other_messages else None,
                'recent_topics': await self._extract_user_topics(other_messages[-3:])  # Last 3 messages
            }
        
        # Get multi-user conversation threads if boundary management enabled
        if self.enable_boundary_management and self.boundary_manager:
            active_user_ids = [str(uid) for uid in user_messages.keys() if not any(msg.author.bot for msg in user_messages[uid])]
    ...
    async def detect_conversation_resumption(self, 
                                           channel, 
                                           user_id: int,
                                           message_content: str,
                                           time_gap_threshold_minutes: int = 30) -> Optional[Dict[str, Any]]:
        """
            ...
            if session:
                ...
                if gap_minutes >= time_gap_threshold_minutes:
                    return {
                        ...
                        'previous_topic': session.current_topic.keywords[:3] if session.current_topic else None,
    ...
    async def suggest_conversation_actions(self, 
                                         channel, 
                                         user_id: int,
    ...
    async def _format_messages_for_context(self, messages: List[discord.Message]) -> List[Dict[str, Any]]:
    ...
    async def _build_context_metadata(self, 
                                    session: ConversationSession, 
                                    boundary_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _generate_conversation_guidance(self, 
                                            session: ConversationSession, 
        ...
        if session.state == ConversationState.RESUMED:
            guidance['special_considerations'] = ['acknowledge_resumption', 'provide_context_bridge']
