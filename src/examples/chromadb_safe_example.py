...
async def example_conversation_fixed():
    ...
    for i, (message, description) in enumerate(conversations, 1):
        ...
        print(f"Relationship: {result['user_profile']['relationship_level']} "
...
async def demonstrate_emotion_system_only():
    ...
    for i, message in enumerate(messages, 1):
        ...
        print(f"   Emotion: {emotion_profile.detected_emotion.value} "
              f"(confidence: {emotion_profile.confidence:.2f})")
        print(f"   Triggers: {', '.join(emotion_profile.triggers) if emotion_profile.triggers else 'none'}")
