...
class GraphIntegratedEmotionManager(EmotionManager):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.graph_sync_mode = os.getenv("GRAPH_SYNC_MODE", "async").lower()  # async, sync, disabled
        ...
        logger.info(f"Graph integration: enabled={self.enable_graph_sync}, mode={self.graph_sync_mode}")
    ...
    async def _sync_to_graph_database(self, profile: UserProfile, 
        ...
        try:
            ...
            if topics:
                await self._link_message_to_topics(profile.user_id, emotion_id, topics, emotion_profile)
    ...
    async def _track_relationship_progression(self, profile: UserProfile):
        ...
        if previous_level != current_level:
            ...
            if graph_connector:
                ...
                logger.info(f"Tracked relationship progression for {user_id}: {previous_level} → {current_level}")
    ...
    async def _extract_topics_from_message(self, message: str) -> List[str]:
        ...
        enhanced_topic_keywords = {
            "work_stress": ["work stress", "job pressure", "deadline", "boss", "overtime", "project stress"],
            "work_success": ["promotion", "raise", "work achievement", "project success", "work proud"],
            "family_positive": ["family time", "family gathering", "parents proud", "family support"],
            "family_conflict": ["family argument", "family problem", "parents upset", "family stress"],
            "health_concern": ["doctor", "medical", "health issue", "pain", "sick", "hospital"],
            "relationship_positive": ["relationship good", "partner", "love", "dating", "anniversary"],
            "relationship_problems": ["breakup", "relationship issue", "fight with", "relationship stress"],
    ...
    async def _link_message_to_topics(self, user_id: str, emotion_id: str, 
                                    topics: List[str], emotion_profile: EmotionProfile):
    ...
    async def get_enhanced_emotion_context(self, user_id: str, current_message: str = "") -> str:
        ...
        try:
            ...
            if emotional_patterns.get("triggers"):
                ...
                negative_triggers = [
                    t for t in emotional_patterns["triggers"] 
    ...
    async def get_contextual_memories_for_prompt(self, user_id: str, message: str, 
                                               limit: int = 5) -> str:
        ...
        try:
            ...
            if memories:
                ...
                for memory in memories[:limit]:
                    ...
                    if summary:
                        emotion_context = f" ({emotion})" if emotion and emotion != "neutral" else ""
        ...
        if self.enable_graph_sync:
            try:
                ...
                if graph_connector:
                    ...
                    health_status["graph_integration"]["status"] = graph_health.get("status", "unknown")
