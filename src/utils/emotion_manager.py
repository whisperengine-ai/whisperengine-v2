...
class RelationshipManager:
    ...
    def detect_personal_info(self, message: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.warning(f"LLM connection failed for personal info detection, returning empty dict: {e}")
            return {}
    ...
    def detect_trust_indicators(self, message: str) -> List[str]:
        ...
        try:
            ...
            filtered_indicators = [indicator.strip() for indicator in trust_indicators if indicator and indicator.strip()]
        ...
        if "personal_info" in requirements:
            ...
            if self.memory_manager:
                try:
                    ...
                    memories = self.memory_manager.retrieve_relevant_memories(profile.user_id, "facts about user", limit=5)
                    user_facts = [m for m in memories if m.get('metadata', {}).get('type') == 'user_fact']
    ...
    def get_next_relationship_level(self, current_level: RelationshipLevel) -> Optional[RelationshipLevel]:
...
class EmotionManager:
    ...
    def __init__(self, llm_client=None, memory_manager=None, use_database: bool = True, postgres_pool=None, phase2_integration=None):
        ...
        self.relationship_manager = RelationshipManager(llm_client=llm_client, memory_manager=memory_manager)
        self.user_profiles: Dict[str, UserProfile] = {}
        self.memory_manager = memory_manager  # Reference to memory system for user facts
        self.phase2_integration = phase2_integration  # Phase 2 Predictive Emotional Intelligence system
        ...
        logger.info(f"EmotionManager initialized with {'LLM-based' if llm_client else 'pattern-based'} sentiment analysis")
            ...
            if self.use_database and self.database:
                try:
                    ...
                    if self._pending_saves:
                        profiles_to_save = {user_id: self.user_profiles[user_id] 
    ...
    def get_or_create_profile(self, user_id: str, display_name: Optional[str] = None) -> UserProfile:
        ...
        if user_id not in self.user_profiles:
            ...
            if display_name and existing_profile.name != display_name:
                logger.debug(f"Updating name for {user_id}: '{existing_profile.name}' -> '{display_name}'")
                existing_profile.name = display_name
        
        return self.user_profiles[user_id]

    def analyze_and_update_emotion(self, user_id: str, message: str, display_name: Optional[str] = None) -> Tuple[UserProfile, EmotionProfile]:
    ...
    async def process_interaction_async(self, user_id: str, message: str, display_name: Optional[str] = None) -> Tuple[UserProfile, EmotionProfile]:
    ...
    def get_emotion_context(self, user_id: str) -> str:
        ...
        if user_id not in self.user_profiles:
            return "User: Stranger, Emotion: Neutral. This is your first interaction with this user."
        ...
        if profile.relationship_level == RelationshipLevel.STRANGER:
            ...
            context_parts.append(f"Be warm and personal. Use their name ({profile.name}) when appropriate.")
            ...
            context_parts.append(f"Be very personal, empathetic, and supportive. You know {profile.name} well.")
        ...
        if profile.escalation_count >= 3:
            context_parts.append("WARNING: User has shown repeated negative emotions. Be extra empathetic and consider offering additional help.")
    ...
    def cleanup_old_data(self, days_to_keep: int = 30):
        ...
        for user_id, profile in self.user_profiles.items():
            ...
            if profile.emotion_history is not None:
                profile.emotion_history = [
                    emotion for emotion in profile.emotion_history 
    ...
    def _get_user_facts_summary(self, user_id: str, detailed: bool = False) -> str:
        ...
        try:
            ...
            facts_query = f"facts about user {user_id}" if detailed else f"basic info about user {user_id}"
            memories = self.memory_manager.retrieve_relevant_memories(user_id, facts_query, limit=10)
...
if __name__ == "__main__":
    ...
    for message in test_messages:
        ...
        print(f"\nMessage: {message}")
        print(f"Detected Emotion: {emotion.detected_emotion.value} (confidence: {emotion.confidence:.2f})")
