            ...
            if hasattr(self.bot, "ws") and self.bot.ws:
                ...
                if latency > 0:
                    ...
                    if latency > 3.0:  # 3+ seconds is very concerning
                        ...
                        logger.warning(
                            f"HIGH heartbeat latency: {latency:.2f}s (issue #{self.connection_issues})"
                        )

                        # Immediate recovery attempt for very high latency
                        if latency > 10.0:
                            logger.error(
                                f"CRITICAL heartbeat latency: {latency:.2f}s - attempting immediate recovery"
                            )
                    ...
                    if self.connection_issues >= self.max_connection_issues:
                        logger.error(
                            "Multiple connection issues detected - attempting proactive recovery"
                        )
                        await self._handle_connection_problem()
                else:
                    ...
                    logger.warning(
                        f"Invalid heartbeat latency: {latency} (issue #{self.connection_issues})"
                    )
        ...
        try:
            ...
            try:
                activity = discord.Activity(
                    type=discord.ActivityType.listening, name="for messages"
                )
