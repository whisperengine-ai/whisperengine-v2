...
class ConversationHistoryManager:
    ...
    def __init__(self, max_channels: int = 100, max_messages_per_channel: int = 20):
        ...
        logger.debug(f"ConversationHistoryManager initialized: max_channels={max_channels}, max_messages={max_messages_per_channel}")
    ...
    def add_message(self, channel_id: str, message: dict):
        ...
        if channel_id not in self.channels and len(self.channels) >= self.max_channels:
            ...
            logger.debug(f"Removed conversation history for channel {oldest_channel} ({len(removed)} messages)")
