...
class PostgreSQLUserDB:
    ...
    async def initialize(self):
        ...
        try:
            ...
            error_msg = (
                f"PostgreSQL authentication failed for user '{self._connection_params['user']}'"
            )
            logger.error(error_msg)
            ...
            logger.error(error_msg)
            logger.info(
                "To fix: Start PostgreSQL with 'docker compose up postgres' or check server status"
            )
            raise ConnectionError(error_msg) from e
            ...
            logger.error(error_msg)
            logger.info(
                "To fix: Start PostgreSQL server or check POSTGRES_HOST/POSTGRES_PORT settings"
            )
            raise ConnectionError(error_msg) from e
        ...
        async with self._lock:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    if not self.pool or (
                        hasattr(self.pool, "is_closing") and self.pool.is_closing()
                    ):
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            f"Database operation timed out after {max_retries} attempts"
                        )
                    ...
                    if "another operation is in progress" in error_msg.lower():
                        logger.warning(
                            f"Connection busy for user {profile.user_id}, attempt {attempt + 1}/{max_retries}"
                        )
                        if attempt < max_retries - 1:
                            await asyncio.sleep(retry_delay * (attempt + 1))
                            continue
