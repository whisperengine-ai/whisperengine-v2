...
class TimeoutLock:
    ...
    def release(self):
        ...
        if self._acquired_by:
            ...
            logger.debug(
                f"Lock '{self.name}' released by {self._acquired_by}, held for {held_time:.2f}s"
            )
            self._acquired_by = None
            self._acquired_at = None
...
class EnhancedAsyncMemoryManager:
            ...
            with self._user_locks_manager:
                if user_id not in self._user_locks:
                    self._user_locks[user_id] = TimeoutLock(
                        timeout=self.lock_timeout, name=f"user_{user_id[:8]}"
                    )
            ...
            with self._emotion_locks_manager:
                if user_id not in self._emotion_locks:
                    self._emotion_locks[user_id] = TimeoutLock(
                        timeout=self.lock_timeout, name=f"emotion_{user_id[:8]}"
                    )
    ...
    async def process_emotion_async(
        ...
        def _process_emotion_with_timeout():
            ...
            try:
                if (
                    hasattr(self.memory_manager, "emotion_manager")
                    and self.memory_manager.emotion_manager
                ):
                    emotion_lock = self._get_emotion_lock(user_id)
                    with emotion_lock:
                        result = self.memory_manager.emotion_manager.process_interaction(
                            user_id, message
                        )
                        self._operation_stats["successful_operations"] += 1
                        return result
        ...
        try:
            return await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(
                    self._executor, _process_emotion_with_timeout
                ),
                timeout=operation_timeout,
            )
        except asyncio.TimeoutError:
    ...
    async def store_global_fact_async(
        self,
        fact: str,
        context: str = "",
        added_by: str = "system",
        ...
        try:
            return await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(
                    self._executor, _store_global_fact_with_timeout
                ),
                timeout=operation_timeout,
            )
        except asyncio.TimeoutError:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._operation_stats,
            ...
            "average_wait_time": (
                self._operation_stats["total_wait_time"]
                / max(1, self._operation_stats["successful_operations"])
            ),
        }
...
class EnhancedConversationCache:
    ...
    def __init__(
        ...
        logger.info(
            f"EnhancedConversationCache initialized with {lock_timeout_seconds}s lock timeout"
        )
            ...
            with self._bootstrap_lock_manager:
                if channel_id not in self._bootstrap_locks:
                    self._bootstrap_locks[channel_id] = TimeoutLock(
                        timeout=self.lock_timeout, name=f"bootstrap_{channel_id[:8]}"
                    )
        ...
        try:
            with self._cache_lock:
                ...
                if len(self._cache[channel_id]["messages"]) > self.max_local_messages:
                    self._cache[channel_id]["messages"] = self._cache[channel_id]["messages"][
                        -self.max_local_messages :
                    ]
            ...
            logger.warning(
                f"Cache lock timeout adding message to channel {channel_id}, skipping cache"
            )
