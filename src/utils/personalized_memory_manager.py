...
class PersonalizedMemoryManager(UserMemoryManager):
    ...
    def __init__(self, *args, enable_graph_memory: bool = True, 
                 graph_config: Optional[GraphMemoryConfig] = None, **kwargs):
    ...
    def store_conversation(self, user_id: str, user_message: str, bot_response: str, 
        ...
        super().store_conversation(user_id, user_message, bot_response, 
                                 channel_id, pre_analyzed_emotion_data, metadata)
        
        # Enhance with graph relationships if enabled
        if self.enable_graph_memory and self.graph_manager:
            asyncio.create_task(self._store_graph_relationships(
                user_id, user_message, bot_response, pre_analyzed_emotion_data
            ))

    async def _store_graph_relationships(self, user_id: str, user_message: str, 
                                       bot_response: str, emotion_data: Optional[dict] = None):
    ...
    def _calculate_memory_importance(self, message: str, emotion_data: Optional[dict] = None) -> float:
    ...
    async def _infer_personality_traits(self, user_id: str) -> List[str]:
        ...
        try:
            ...
            memories = self.retrieve_relevant_memories(user_id, "personality communication style", limit=20)
    ...
    async def get_personalized_context(self, user_id: str, query: str, limit: int = 5) -> str:
        ...
        try:
            ...
            related_memories = await self.graph_manager.find_related_memories(user_id, topics, limit)
    ...
    async def track_relationship_milestone(self, user_id: str, milestone_type: str, context: Optional[str] = None):
