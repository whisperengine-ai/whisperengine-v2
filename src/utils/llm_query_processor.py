...
class LLMQueryProcessor:
    ...
    async def analyze_message_for_search(self, message: str, conversation_context: Optional[str] = None) -> LLMQueryBreakdown:
        """
    ...
    async def _call_llm_for_query_breakdown(self, message: str, context_prompt: str) -> Dict[str, Any]:
...
- Lower weight (0.5-0.8) for supplementary queries"""
    ...
    def _parse_llm_response(self, response: Dict[str, Any], original_message: str) -> LLMQueryBreakdown:
        """Parse LLM response into structured breakdown"""
        ...
        if not primary_queries:
            primary_queries = [LLMSearchQuery(
    ...
    def _clean_message_for_fallback(self, message: str) -> str:
        ...
        stop_words = {
            'i', 'me', 'my', 'myself', 'am', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
            'the', 'a', 'an', 'and', 'or', 'but', 'if', 'then', 'when', 'how', 'what', 'why',
            'do', 'does', 'did', 'will', 'would', 'can', 'could', 'should', 'just', 'really',
            'tell', 'help', 'please', 'thanks', 'you', 'your', 'today', 'now'
        }
...
class HybridQueryProcessor:
    ...
    async def process_message(self, message: str, conversation_context: Optional[str] = None) -> LLMQueryBreakdown:
        """
        ...
        if self.enable_llm and self.llm_processor:
            try:
                ...
                result = await self.llm_processor.analyze_message_for_search(message, conversation_context)
                
                # Validate that we got meaningful queries
                if result.primary_queries and any(len(q.query.strip()) > 3 for q in result.primary_queries):
                    logger.debug(f"LLM query breakdown successful: {len(result.primary_queries)} queries generated")
                    return result
                else:
    ...
    def _clean_message_simple(self, message: str) -> str:
        ...
        message = re.sub(r'\b(can you|could you|please|help me|tell me|what about|how about)\b', '', message, flags=re.IGNORECASE)
        
        # Extract meaningful words
        words = re.findall(r'\b\w{3,}\b', message.lower())
        
        # Remove common stop words
        stop_words = {'the', 'and', 'for', 'are', 'but', 'not', 'you', 'all', 'can', 'her', 'was', 'one', 'our', 'had', 'but', 'not', 'what', 'all', 'were', 'they', 'said', 'each', 'which', 'she', 'their', 'time', 'will', 'about', 'if', 'up', 'out', 'many', 'then', 'them', 'these', 'so', 'some', 'would', 'other', 'into', 'has', 'more', 'go', 'no', 'way', 'could', 'my', 'than', 'first', 'been', 'call', 'who', 'its', 'now', 'find', 'long', 'down', 'day', 'did', 'get', 'come', 'made', 'may', 'part'}
