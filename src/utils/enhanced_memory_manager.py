...
class EnhancedMemoryManager:
    ...
    def retrieve_relevant_memories_enhanced(self, user_id: str, message: str, 
                                          limit: int = 10, 
                                          context: Optional[Any] = None) -> List[Dict]:
        """
    ...
    def _retrieve_with_combined_queries(self, user_id: str, 
                                      search_strategies: List[Tuple[str, float]], 
                                      limit: int, context: Optional[Any],
                                      query_result: QueryResult) -> List[Dict]:
        """
            ...
            try:
                ...
                if hasattr(self.base_memory_manager, 'retrieve_context_aware_memories') and context:
                    memories = self.base_memory_manager.retrieve_context_aware_memories(
                        user_id, query, context, limit=limit * 2  # Get more to allow for combination
                    )
                else:
                ...
                query_count += 1
                logger.debug(f"Query '{query}' (weight: {weight}) returned {len(memories)} memories")
        ...
        if combined_memories:
            ...
            logger.debug(f"Top result: score={top_memory.get('enhanced_score', 0):.3f}, "
                        f"matches={top_memory.get('query_matches', 0)}, "
                        f"query='{top_memory.get('matching_query', 'unknown')}'")
        
        return combined_memories[:limit]

    def _retrieve_with_sequential_queries(self, user_id: str, 
                                        search_strategies: List[Tuple[str, float]], 
                                        limit: int, context: Optional[Any],
                                        query_result: QueryResult) -> List[Dict]:
        """
        ...
        for query, weight in search_strategies:
            ...
            try:
                ...
                logger.debug(f"Query '{query}' added {len([m for m in memories if m.get('id') not in seen_ids])} new memories")
    ...
    def _fallback_retrieve(self, user_id: str, message: str, limit: int, 
    ...
    def retrieve_context_aware_memories(self, user_id: str, message: str, 
                                      context: Any, limit: int = 10) -> List[Dict]:
        """
        Enhanced version of context-aware memory retrieval
        
        This replaces the original method with our enhanced query processing
        """
        return self.retrieve_relevant_memories_enhanced(
            user_id, message, limit, context
        )

    def retrieve_relevant_memories(self, user_id: str, message: str, 
