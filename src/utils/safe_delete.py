...
class SafeUserDeletion:
    ...
    def preview_user_deletion(self, user_id: str) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Get emotion count
            emotion_cursor = conn.execute("SELECT COUNT(*) FROM emotion_history WHERE user_id = ?", (user_id,))
    ...
    def safe_cleanup_emotions(self, days: int, auto_backup: bool = True) -> bool:
        ...
        if preview["sample_records"]:
            ...
            for record in preview["sample_records"][:5]:
                print(f"   • {record['user_id']}: {record['detected_emotion']} at {record['timestamp']}")
    ...
    def safe_reset_user(self, user_id: str, full_reset: bool = False, auto_backup: bool = True) -> bool:
    ...
    try:
        if command == "delete" and len(sys.argv) > 2:
            ...
            if subcommand == "user":
                ...
                print(f"Would delete {preview['count']} records affecting {preview['affected_users']} users")
