...
class HealthCheckServer:
    ...
    async def readiness_check(self, request):
        ...
        try:
            ...
            if is_ready:
                ...
                logger.warning(f"Readiness check failed - bot not ready: ready={self.bot.is_ready()}, closed={self.bot.is_closed()}")
                return web.json_response({
                    "status": "not_ready",
                    "timestamp": datetime.utcnow().isoformat(),
    ...
    async def metrics(self, request):
        ...
        try:
            metrics_data = {
                "timestamp": datetime.utcnow().isoformat(),
                "bot_latency_ms": round(self.bot.latency * 1000, 2) if self.bot.is_ready() else None,
...
def create_health_server(bot: commands.Bot, port: int = 9090, host: str = "0.0.0.0") -> HealthCheckServer:
