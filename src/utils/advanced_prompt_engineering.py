...
class AdvancedPromptEngineer:
    ...
    def optimize_prompt_for_user(
        ...
        system_prompt = self._build_optimized_system_prompt(
            user_profile, message_analysis, context
        )
    ...
    def _analyze_message(self, message: str) -> Dict[str, Any]:
        ...
        elif any(word in message_lower for word in ['feel', 'emotion', 'sad', 'happy', 'angry', 'excited']):
            message_type = "emotional"
        elif any(word in message_lower for word in ['remember', 'recall', 'mentioned', 'talked about']):
            message_type = "recall"
...
✓ Search strategy matches message characteristics"""
    ...
    def _get_message_specific_instructions(self, message_analysis: Dict[str, Any]) -> str:
        ...
        if message_analysis["type"] == "question":
            instructions.append("• Focus on the subject matter of the question, not the questioning itself")
            instructions.append("• Extract the core topic being asked about")
        
        if message_analysis["type"] == "emotional":
            instructions.append("• Include emotional keywords that would match past emotional discussions")
        ...
        if message_analysis["type"] == "recall":
            ...
            instructions.append("• Create queries that would match the remembered content, not the act of remembering")
        ...
        if message_analysis["complexity"] == "complex":
            instructions.append("• Break down into multiple focused queries covering different aspects")
        ...
        if not instructions:
            instructions.append("• Create focused queries that capture the main themes and concepts")
    ...
    def _compress_system_prompt(self, prompt: str) -> str:
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in [
                'analyze', 'generate', 'extract', 'focus', 'prioritize', 'rules:', 'format:'
            ]):
...
def create_optimized_prompt_engineer(
    ...
    logger.info(f"Advanced prompt engineer created with features: "
