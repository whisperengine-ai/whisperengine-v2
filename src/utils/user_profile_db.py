...
class UserProfileDatabase:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_user_last_interaction ON users(last_interaction)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_emotion_history_user_time ON emotion_history(user_id, timestamp)"
            )
    ...
    def load_user_profile(self, user_id: str) -> UserProfile | None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                trust_indicators = (
                    json.loads(trust_indicators_data) if trust_indicators_data else []
                )
                # Ensure it's a list
    ...
    def delete_user_emotions(self, user_id: str, older_than_days: int | None = None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            if older_than_days:
                ...
                logger.info(
                    f"Deleted {deleted.rowcount} emotion records older than {older_than_days} days for user {user_id}"
                )
            else:
    ...
    def reset_user_profile(self, user_id: str, keep_basic_info: bool = True):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            emotion_deleted = conn.execute(
                "DELETE FROM emotion_history WHERE user_id = ?", (user_id,)
            )
            ...
            if keep_basic_info:
                ...
                logger.info(
                    f"Reset user profile for {user_id} (kept basic info, deleted {emotion_deleted.rowcount} emotions)"
                )
            else:
                # Delete everything
                user_deleted = conn.execute("DELETE FROM users WHERE user_id = ?", (user_id,))
                logger.info(
                    f"Completely deleted user {user_id} ({user_deleted.rowcount} user record, {emotion_deleted.rowcount} emotions)"
                )
    ...
    def archive_inactive_users(self, days_inactive: int = 90):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            logger.info(
                f"Archived {archived.rowcount} inactive users (inactive for {days_inactive}+ days)"
            )
        ...
        try:
            ...
            for user_id, profile_data in data.items():
                try:
                    ...
                    profile_data["relationship_level"] = RelationshipLevel(
                        profile_data["relationship_level"]
                    )
                    profile_data["current_emotion"] = EmotionalState(
                        profile_data["current_emotion"]
                    )

                    # Convert emotion history
                    emotion_history = []
                    for emotion_data in profile_data.get("emotion_history", []):
                        emotion_data["detected_emotion"] = EmotionalState(
                            emotion_data["detected_emotion"]
                        )
            ...
            logger.info(
                f"Successfully migrated {migrated_count} user profiles from JSON to database"
            )
