...
class ImageProcessor:
    ...
    def is_supported_image(self, filename: str) -> bool:
        ...
        self.logger.debug(
            f"Image support check for '{filename}': {is_supported} (extension: {extension})"
        )
        return is_supported
                ...
                async with session.get(url) as response:
                    ...
                    async for chunk in response.content.iter_chunked(8192):
                        ...
                        if len(image_data) > max_size:
                            self.logger.warning(
                                f"Image exceeded size limit during download: {len(image_data)} bytes"
                            )
                            return None
    ...
    async def process_discord_attachment(self, attachment) -> Optional[Dict[str, Any]]:
        ...
        self.logger.info(
            f"Processing image attachment: {attachment.filename} ({attachment.size} bytes)"
        )
    ...
    async def process_multiple_attachments(self, attachments) -> List[Dict[str, Any]]:
        ...
        self.logger.info(
            f"Processed {len(processed_images)} images from {len(attachments)} attachments"
        )
