...
class HealthMonitor:
    ...
    def __init__(self, memory_manager, llm_client, conversation_cache, 
                 emotion_manager=None, backup_manager=None):
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await self.memory_manager.retrieve_memories_safe("health_check", "test", limit=1)
        ...
        if self.backup_manager:
            try:
                health_status["components"]["backup"] = {
                    "status": "healthy",
                    "auto_backup_enabled": getattr(self.backup_manager, 'auto_backup_enabled', False)
                }
            except Exception as e:
                health_status["components"]["backup"] = {
                    "status": "error",
        ...
        error_components = [name for name, status in health_status["components"].items() 
