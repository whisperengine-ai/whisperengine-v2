...
class ConcurrentLLMManager:
    ...
    async def _check_connection_cached(self) -> bool:
        ...
        try:
            status = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(self._executor, _check_connection),
                timeout=10.0,
            )
    ...
    def has_vision_support(self) -> bool:
        ...
        return (
            hasattr(self.base_llm_client, "has_vision_support")
            and self.base_llm_client.has_vision_support()
        )
