...
class Phase43ProactiveEngagementDemo:
    ...
    async def initialize_systems(self):
        ...
        if ENGAGEMENT_ENGINE_AVAILABLE:
            ...
            logger.info(
                "✅ Phase 4.3 Proactive Engagement Engine initialized with full integration"
            )
        else:
    ...
    async def run_scenario_demo(self, scenario: dict[str, Any]) -> dict[str, Any]:
        ...
        for i, message_data in enumerate(scenario["messages"]):
            ...
            engagement_result = await self.engagement_engine.analyze_conversation_engagement(
                user_id=scenario["user_id"], context_id="demo_context", recent_messages=all_messages
            )
    ...
    def _record_engagement_analysis(
        ...
        for rec in recommendations:
            recommendation_data = {
                ...
                "strategy": (
                    rec.get("strategy", {}).value
                    if hasattr(rec.get("strategy", {}), "value")
                    else str(rec.get("strategy", "unknown"))
                ),
                "content": rec.get("content", ""),
                "engagement_potential": rec.get(
                    "engagement_potential", rec.get("engagement_boost", 0.0)
                ),
            }
    ...
    def _display_engagement_state(self, engagement_result: dict[str, Any], message_num: int):
        ...
        logger.info(
            "   %s Flow: %s | Engagement: %.2f | Stagnation Risk: %.2f",
            emoji,
            flow_state,
            engagement_score,
            stagnation_risk,
    ...
    def _analyze_scenario_performance(
        ...
        logger.info(
            "   Interventions triggered: %d", len(scenario_results["interventions_triggered"])
        )
        logger.info(
            "   Recommendations generated: %d", len(scenario_results["recommendations_generated"])
        )
        logger.info(
            "   Proactive strategies used: %s",
            ", ".join(scenario_results["proactive_strategies_used"]),
        )
        ...
        if expected_strategies:
            ...
            strategies_matched = any(
                strategy in str(strategies_used) for strategy in expected_strategies
            )
    ...
    async def run_integration_demo(self) -> dict[str, Any]:
        ...
        if self.thread_manager:
            try:
                ...
                test_messages = [
                    {
                        "content": "I want to discuss my career goals and programming projects.",
                        "timestamp": datetime.now().isoformat(),
                    },
                    {
                        "content": "Yeah, coding is interesting.",
    ...
    def _display_demo_summary(self, results: dict[str, Any]):
        ...
        successful_scenarios = sum(
            1 for s in scenario_results if s.get("engagement_success", False)
        )

        logger.info(
            "🎭 Conversation Scenarios: %d/%d successful",
        ...
        logger.info("")
        logger.info(
            "🔗 System Integration: %d/%d components integrated",
        ...
        logger.info(
            "   Unique strategies employed: %d (%s)",
            len(total_strategies),
            ", ".join(total_strategies),
        )
        logger.info(
            "   System availability: %d/%d systems",
            sum(results["systems_available"].values()),
            len(results["systems_available"]),
        )
