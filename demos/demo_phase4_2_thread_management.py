    ...
    def _analyze_message_result(
        ...
        if priorities:
            high_priority_threads = [
                thread_id
                for thread_id, priority_data in priorities.items()
    ...
    def _display_thread_state(self, result: dict[str, Any]):
        ...
        if active_threads:
            ...
            for thread in active_threads[:3]:  # Show top 3
                logger.info(
                    f"     - {thread['thread_id']}: {', '.join(thread['topic_keywords'][:3])}"
                )
                logger.info(
                    f"       Priority: {thread['priority_level']}, Engagement: {thread['engagement_level']:.2f}"
                )
        ...
        if guidance and "thread_context" in guidance:
            ...
            logger.info(
                f"  🎯 Response guidance: {context.get('conversation_phase', 'unknown')} phase"
            )
        ...
        for scenario in self.conversation_scenarios:
            try:
                ...
                scenario_results.append(
                    {"scenario_name": scenario["scenario_name"], "error": str(e), "success": False}
                )
    ...
    def _display_demo_summary(self, results: dict[str, Any]):
        ...
        for scenario in scenario_results:
            if scenario.get("success", True):
                ...
                logger.info(
                    f"   ❌ {scenario['scenario_name']}: {scenario.get('error', 'Unknown error')}"
                )
