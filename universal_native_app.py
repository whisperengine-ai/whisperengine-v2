...
try:
    from PySide6.QtWidgets import (
        ...
        QFrame, QSizePolicy, QSystemTrayIcon, QMenu, QMessageBox,
        QDialog, QTabWidget
    )
    ...
    from PySide6.QtGui import (
        QFont, QTextCursor, QIcon, QPixmap, QPalette, QColor,
        QFontDatabase, QAction, QTextOption
    )
...
class WhisperEngineUniversalApp(QMainWindow):
    ...
    def handle_onboarding(self):
        ...
        try:
            ...
            if completed and user_config:
                ...
                if llm_config:
                    logger.info(f"✅ LLM configured: {llm_config.get('server_name')} at {llm_config.get('api_url')}")
            else:
                # Load existing user config
                self.current_user_config = get_current_user_config()
                if self.current_user_config:
                    self.user_id = self.current_user_config.get('user_id')
                    logger.info(f"✅ Loaded existing user: {self.current_user_config.get('username')}")
                else:
    ...
    def init_ui(self):
        ...
        if logo_pixmap:
            ...
            logo_label.setText("🤖")
            logo_label.setStyleSheet("font-size: 32px; margin-right: 12px; margin-left: 4px;")  # Bigger emoji too
    ...
    def create_character_prompt_tab(self):
        ...
        # Info section
        info_label = QLabel("Edit the system prompt that defines the AI's personality, behavior, and knowledge. Changes take effect immediately.")
    ...
    def _detect_macos_gpu(self) -> str:
        ...
        try:
            ...
            result = subprocess.run(['ioreg', '-r', '-n', 'AGXAccelerator'], 
            ...
            try:
                ...
                for gpu in c.Win32_VideoController():
                    if gpu.Name and gpu.AdapterRAM:
                        ...
                        gpu_name = str(gpu.Name)[:20] + "..." if len(str(gpu.Name)) > 20 else str(gpu.Name)
    ...
    def update_resource_stats(self):
        ...
        try:
            ...
            cpu_color = "#00ff00" if self.latest_cpu < 50 else "#ffaa00" if self.latest_cpu < 80 else "#ff0000"
            ...
            mem_color = "#00ff00" if self.latest_memory < 60 else "#ffaa00" if self.latest_memory < 85 else "#ff0000"
            ...
            if self.latest_gpu != "N/A":
                try:
                    ...
                    if '%' in self.latest_gpu:
                        ...
                        gpu_color = "#00ff00" if gpu_val < 50 else "#ffaa00" if gpu_val < 80 else "#ff0000"
    ...
    def update_system_status(self, status: str, icon: str = "🟢", color: str = "#00ff00"):
        ...
        if hasattr(self, 'system_status_icon') and hasattr(self, 'system_status_text'):
            ...
            self.system_status_text.setStyleSheet(f"color: {color}; font-size: 10px; font-weight: bold;")
    ...
    def update_llm_indicator(self):
        ...
        try:
            ...
            if api_url and model_name:
                ...
                self.llm_indicator.setStyleSheet("color: #00aa00; font-size: 11px; font-style: normal;")
                logger.debug(f"LLM Indicator updated: {endpoint_name} • {display_model}")
            else:
                self.llm_indicator.setText("🔗 LLM Not Configured")
                self.llm_indicator.setStyleSheet("color: #aa0000; font-size: 11px; font-style: italic;")
                logger.debug(f"LLM Indicator: Not configured (API URL: '{api_url}', Model: '{model_name}')")
        ...
        def init_service():
            try:
                ...
                if success:
                    ...
                    QTimer.singleShot(0, lambda: self.on_ai_service_error("AI service initialization returned False"))
