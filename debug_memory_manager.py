...
def debug_memory_manager():
    ...
    try:
        ...
        print(f"  use_external_embeddings: {getattr(memory_manager, 'use_external_embeddings', 'NOT_SET')}")
        print(f"  add_documents_with_embeddings: {getattr(memory_manager, 'add_documents_with_embeddings', 'NOT_SET')}")
        print(f"  query_with_embeddings: {getattr(memory_manager, 'query_with_embeddings', 'NOT_SET')}")
        
        # Check the condition
        condition_result = getattr(memory_manager, 'use_external_embeddings', False) and getattr(memory_manager, 'add_documents_with_embeddings', None)
