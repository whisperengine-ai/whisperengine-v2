#!/usr/bin/env python3
"""
IMPROVED WhisperEngine Vector Storage Migration: Remove bot_name Filtering
...
1. True batch processing of point updates
2. More efficient payload management
3. More thorough collection handling
4. Clearer progress reporting
...
class VectorStorageMigration:
    ...
    async def run_full_migration(self) -> Dict[str, Any]:
        ...
        logger.info("🚀 Starting IMPROVED WhisperEngine Vector Storage Migration")
    ...
    async def migrate_collection(self, collection_name: str) -> Dict[str, Any]:
        """Migrate a single collection to remove bot_name fields - IMPROVED BATCH PROCESSING"""
        ...
        try:
            ...
            while True:
                ...
                # IMPROVEMENT: Process points in true batches
                point_ids_with_bot_name = []
                updated_payloads = {}
                ...
                for point in points:
                    ...
                    if point.payload and 'bot_name' in point.payload:
                        # Create new payload without bot_name
                        new_payload = {k: v for k, v in point.payload.items() if k != 'bot_name'}
                        
                        # Only include points that actually need updating
                        if len(new_payload) != len(point.payload):
                            point_ids_with_bot_name.append(point.id)
                            updated_payloads[point.id] = new_payload
                
                # Process batch update if we have points to update
                if point_ids_with_bot_name:
                    try:
                        # IMPROVEMENT: Use delete_payload to specifically target bot_name field
                        self.client.delete_payload(
                            collection_name=collection_name,
                            points=point_ids_with_bot_name,
                            keys=["bot_name"],
                            wait=True
                        )
                        
                        # Update our stats
                        batch_updates = len(point_ids_with_bot_name)
                        stats['points_updated'] += batch_updates
                        stats['bot_names_removed'] += batch_updates
                        
                        logger.info(f"  ✅ Batch update: Removed bot_name from {batch_updates} points")
                    except Exception as e:
                        error_msg = f"Failed to update batch of {len(point_ids_with_bot_name)} points: {str(e)}"
    ...
    async def delete_payload_field(self, collection_name: str, field_name: str) -> Dict[str, Any]:
        """Delete a specific field from all payloads in a collection."""
        logger.info(f"🗑️  Deleting field '{field_name}' from all points in {collection_name}")
        ...
        try:
            ...
            # Delete field from all points at once
            if all_points:
                result = self.client.delete_payload(
                    collection_name=collection_name,
                    points=all_points,
                    keys=[field_name],
                    wait=True
                ...
                return {
                    ...
                    'field': field_name,
                    'points_affected': len(all_points),
                    'status': 'success'
                }
            else:
                return {
            ...
            logger.error(f"❌ Error deleting field '{field_name}' from {collection_name}: {e}")
            return {
                'collection': collection_name,
                'field': field_name,
                'status': 'error',
    ...
    async def validate_migration(self) -> Dict[str, Any]:
        ...
        for collection_name in self.collections:
            try:
                # Check for any remaining bot_name fields - improved to check more points
                total_checked = 0
                remaining_bot_names = 0
                offset = None
                batch_size = 250
                max_to_check = 1000  # Check up to 1000 points per collection
                
                while total_checked < max_to_check:
                    scroll_result = self.client.scroll(
                        collection_name=collection_name,
                        limit=batch_size,
                        offset=offset,
                    ...
                    # Check points for bot_name field
                    for point in points:
                        total_checked += 1
                        if point.payload and 'bot_name' in point.payload:
                            remaining_bot_names += 1
                    
                    # Break if we've checked enough points or reached the end
                    if total_checked >= max_to_check or offset is None:
                        break
                
                validation_results[collection_name] = {
                    'remaining_bot_names': remaining_bot_names,
                    'points_checked': total_checked,
                    'clean': remaining_bot_names == 0
                }
                
                if remaining_bot_names == 0:
                    clean_collections += 1
                    logger.info(f"  ✅ {collection_name}: CLEAN - No bot_name fields found in {total_checked} points")
                else:
                    logger.warning(f"  ⚠️ {collection_name}: {remaining_bot_names} bot_name fields found in {total_checked} points")
                    
                    # If validation shows remaining bot_name fields, try to force delete them
                    if remaining_bot_names > 0:
                        logger.info(f"  🔄 Attempting forced cleanup of bot_name fields in {collection_name}")
                        cleanup_result = await self.delete_payload_field(collection_name, "bot_name")
                        validation_results[collection_name]['forced_cleanup'] = cleanup_result
        ...
        # Summary
        logger.info(f"\n🎯 VALIDATION SUMMARY:")
        logger.info(f"  📊 Total collections checked: {len(self.collections)}")
        logger.info(f"  ✅ Clean collections: {clean_collections}")
        
        if clean_collections == len(self.collections):
            logger.info(f"  🚀 ALL COLLECTIONS SUCCESSFULLY MIGRATED!")
        else:
            failed = len(self.collections) - clean_collections
            logger.warning(f"  ⚠️ {failed} collections needed additional cleanup")
...
async def main():
    ...
    print("🚀 IMPROVED WhisperEngine Vector Storage Migration")
    ...
    try:
        ...
        all_clean = all(
            ...
            if isinstance(result, dict) and 'clean' in result
        )
        
        if all_clean:
            print("✅ All collections successfully migrated - no bot_name fields remaining")
        else:
            print("⚠️ Some collections required additional cleanup - check validation results")
