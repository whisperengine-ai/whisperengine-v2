...
if [ -f "$BACKUP_DIR/postgres.dump" ]; then
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${POSTGRES_CONTAINER}$"; then
        ...
        # Restore from dump
        cat "$BACKUP_DIR/postgres.dump" | docker exec -i "$POSTGRES_CONTAINER" pg_restore -U "$POSTGRES_USER" -d "$POSTGRES_DB" --no-owner --no-acl 2>/dev/null || true
...
if [ -f "$BACKUP_DIR/qdrant_snapshot.tar" ]; then
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${QDRANT_CONTAINER}$"; then
        ...
        docker cp "$BACKUP_DIR/qdrant_snapshot.tar" "$QDRANT_CONTAINER:/qdrant/snapshots/restore.tar"
        
        # Restore via API
        curl -s -X POST "http://localhost:6333/snapshots/recover" \
            -H "Content-Type: application/json" \
            -d "{\"location\": \"file:///qdrant/snapshots/restore.tar\"}" > /dev/null
        
        # Cleanup
        docker exec "$QDRANT_CONTAINER" rm -f /qdrant/snapshots/restore.tar
...
if [ -f "$BACKUP_DIR/neo4j.cypher" ]; then
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${NEO4J_CONTAINER}$"; then
        ...
        # Restore from cypher statements
        cat "$BACKUP_DIR/neo4j.cypher" | docker exec -i "$NEO4J_CONTAINER" cypher-shell -u neo4j -p password 2>/dev/null || true
        echo -e "${GREEN}  ✓ Neo4j restored${NC}"
    else
        echo -e "${RED}  ✗ Neo4j container not running${NC}"
    fi
elif [ -f "$BACKUP_DIR/neo4j_nodes.txt" ]; then
    echo -e "${YELLOW}[3/4] Neo4j backup is in basic format - manual restore required${NC}"
    echo -e "  Files: $BACKUP_DIR/neo4j_nodes.txt, $BACKUP_DIR/neo4j_rels.txt"
else
...
if [ -d "$BACKUP_DIR/influxdb" ]; then
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${INFLUXDB_CONTAINER}$"; then
        ...
        docker cp "$BACKUP_DIR/influxdb" "$INFLUXDB_CONTAINER:/tmp/influx_restore"
        
        # Restore using influx CLI
        docker exec "$INFLUXDB_CONTAINER" influx restore /tmp/influx_restore \
            --org whisperengine \
            --token my-super-secret-auth-token \
            --full 2>/dev/null || true
        
        # Cleanup
        docker exec "$INFLUXDB_CONTAINER" rm -rf /tmp/influx_restore
