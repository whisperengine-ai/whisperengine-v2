...
class ChatGPTImporter:
    def __init__(
        ...
        enable_auto_facts: bool = True,
        enable_global_facts: bool = False,
        llm_client=None,
    ):
            ...
            if llm_client is None:
                try:
                    ...
                    if not llm_client.check_connection():
                        logger.warning(
                            "LLM Studio server not available - fact extraction and emotion analysis will be disabled"
                        )
                        llm_client = None
                except Exception as e:
                    logger.warning(
                        f"Failed to initialize LLM client: {e} - fact extraction and emotion analysis will be disabled"
                    )
                    llm_client = None
    ...
    def parse_conversations(self, export_data: dict) -> list[dict]:
        ...
        if isinstance(export_data, list):
            ...
            logger.info(
                f"Found nested conversations format with {len(raw_conversations)} conversations"
            )
                ...
                if message_data.get("content") and message_data.get("author"):
                    ...
                    if isinstance(content, dict):
                        ...
                        # Text content format
                            ...
                            text_content = (
                                str(content["text"]) if content["text"] is not None else ""
                            )
                        # Direct content
                        elif (
                            isinstance(content.get("content_type"), str)
    ...
    def import_conversations(
        ...
        for i, conversation in enumerate(conversations, 1):
            logger.info(
                f"Processing conversation {i}/{len(conversations)}: {conversation.get('title', 'Untitled')}"
            )
            ...
            for turn_idx, (user_msg, assistant_msg) in enumerate(turns):
                try:
                    ...
                    logger.debug(
                        f"Imported turn {turn_idx + 1} from conversation {conversation['id']}"
                    )

                except Exception as e:
                    stats["errors"] += 1
                    logger.warning(
                        f"Error importing turn from conversation {conversation['id']}: {e}"
                    )
                    continue
...
Examples:
  ...
  %(prog)s 123456789012345678 export.json --enable-global-facts
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview import without actually storing data"
    )
    parser.add_argument(
        "--no-auto-facts",
        action="store_true",
