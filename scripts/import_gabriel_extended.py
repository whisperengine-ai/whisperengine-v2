#!/usr/bin/env python3
"""
Import Gabriel Extended Data with Semantic Mapping
==================================================

Gabriel uses NESTED character object format (different from Elena's flat structure).
This script extracts extended data from Gabriel's JSON and imports to RDBMS.

Target: ~47 extended records total
Current: 12 records (5 emoji, 1 expertise, 6 conversation_flows)
Missing: ~35 records (message_triggers, cultural_expressions, emotional_triggers, voice_traits, response_guidelines, ai_scenarios, expertise_domains)
...
- emotional_profile.triggers → character_emotional_triggers (additional)
- speech_patterns → character_voice_traits
- behavioral_patterns.interaction_guidelines → character_response_guidelines
- behavioral_patterns.response_patterns → character_ai_scenarios
- capabilities.knowledge_domains → character_expertise_domains
...
async def import_gabriel_extended():
    """Import Gabriel's missing extended data from JSON."""
    
    # Load Gabriel's JSON
    gabriel_json_path = Path(__file__).parent.parent / "characters" / "examples_legacy_backup" / "gabriel.json"
    with open(gabriel_json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
        gabriel = data['character']
    
    print(f"{'='*70}")
    print("🔧 GABRIEL EXTENDED DATA IMPORT (SEMANTIC MAPPING)")
    print(f"{'='*70}\n")
    ...
    try:
        ...
        result = await conn.fetchrow(
            "SELECT id FROM characters WHERE name = 'Gabriel' LIMIT 1"
        )
        ...
        print(f"✅ Found Gabriel (ID: {character_id})\n")
        
        total_imported = 0
        
        # 1. MESSAGE TRIGGERS from communication.message_pattern_triggers
        print("📨 Importing MESSAGE TRIGGERS...")
        if 'communication' in gabriel and 'message_pattern_triggers' in gabriel['communication']:
            triggers_dict = gabriel['communication']['message_pattern_triggers']
        ...
        if 'communication' in gabriel and 'typical_responses' in gabriel['communication']:
            responses = gabriel['communication']['typical_responses']
        ...
        if 'communication' in gabriel and 'emotional_expressions' in gabriel['communication']:
            emotions = gabriel['communication']['emotional_expressions']
        ...
        if 'speech_patterns' in gabriel:
            sp = gabriel['speech_patterns']
                ...
                if trait_name == 'vocabulary' and isinstance(trait_value, dict):
                    ...
                    if 'preferred_words' in trait_value and isinstance(trait_value['preferred_words'], list):
                        for word in trait_value['preferred_words']:
                            await conn.execute("""
                                ...
                                VALUES ($1, $2, $3, $4)
                            """, character_id, 'preferred_word', word, "Words Gabriel naturally uses")
                            total_imported += 1
                    ...
                    if 'avoided_words' in trait_value and isinstance(trait_value['avoided_words'], list):
                        for word in trait_value['avoided_words']:
                            await conn.execute("""
                                ...
                                (character_id, trait_type, trait_value, situational_context)
                                VALUES ($1, $2, $3, $4)
                            """, character_id, 'avoided_word', word, "Words Gabriel avoids")
                            total_imported += 1
                    
                    # Extract grounding_phrases (string)
                    if 'grounding_phrases' in trait_value and isinstance(trait_value['grounding_phrases'], str):
                        await conn.execute("""
                            INSERT INTO character_voice_traits 
                            (character_id, trait_type, trait_value, situational_context)
                            VALUES ($1, $2, $3, $4)
                        """, character_id, 'grounding_phrases', trait_value['grounding_phrases'], "Gabriel's grounding phrases")
                        total_imported += 1
                    
                    # Extract signature_expressions (list)
                    if 'signature_expressions' in trait_value and isinstance(trait_value['signature_expressions'], list):
                        for expr in trait_value['signature_expressions']:
                            await conn.execute("""
                                INSERT INTO character_voice_traits 
                                (character_id, trait_type, trait_value, situational_context)
                                VALUES ($1, $2, $3, $4)
                            """, character_id, 'signature_expression', expr, "Gabriel's signature expressions")
                            total_imported += 1
                    
                    print(f"  ✅ vocabulary: extracted {len(trait_value.get('preferred_words', []))} preferred, {len(trait_value.get('avoided_words', []))} avoided, {len(trait_value.get('signature_expressions', []))} signature expressions")
                else:
                    ...
                    if isinstance(trait_value, str):
                        await conn.execute("""
                            ...
                            (character_id, trait_type, trait_value, situational_context)
                            VALUES ($1, $2, $3, $4)
                        """, character_id, trait_name, trait_value, f"Gabriel's {trait_name.replace('_', ' ')}")
        ...
        if 'behavioral_patterns' in gabriel and 'interaction_guidelines' in gabriel['behavioral_patterns']:
            guidelines = gabriel['behavioral_patterns']['interaction_guidelines']
        ...
        if 'communication' in gabriel and 'response_length' in gabriel['communication']:
            response_length = gabriel['communication']['response_length']
        ...
        if 'behavioral_patterns' in gabriel and 'response_patterns' in gabriel['behavioral_patterns']:
            patterns = gabriel['behavioral_patterns']['response_patterns']
        ...
        # 7. EXPERTISE DOMAINS from capabilities.knowledge_domains
        print("\n🎓 Importing EXPERTISE DOMAINS...")
        if 'capabilities' in gabriel and 'knowledge_domains' in gabriel['capabilities']:
            domains = gabriel['capabilities']['knowledge_domains']
            ...
            for domain in domains:
                await conn.execute("""
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                """, character_id, domain, 'expert', 90, f"Gabriel's expertise in {domain}")
                
                print(f"  ✅ {domain}")
                total_imported += 1
        
        # 8. EMOTIONAL TRIGGERS from emotional_profile.triggers (additional)
        print("\n💫 Importing ADDITIONAL EMOTIONAL TRIGGERS...")
        if 'emotional_profile' in gabriel and 'triggers' in gabriel['emotional_profile']:
            triggers = gabriel['emotional_profile']['triggers']
...
if __name__ == "__main__":
    asyncio.run(import_gabriel_extended())
