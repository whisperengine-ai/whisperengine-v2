#!/usr/bin/env python3
"""
Synchronous ChatGPT Import Script
Avoids async conflicts with WhisperEngine memory system
"""
...
class SyncChatGPTImporter:
    """Synchronous ChatGPT conversation importer"""
    ...
    def import_conversations(self, file_path: str) -> Dict[str, int]:
        ...
        if not self.dry_run:
            logger.info("Import complete - analyzing imported data")
        else:
    ...
    def _process_conversation(self, conversation: Dict, index: int):
        ...
        try:
            ...
            if self.verbose:
                logger.debug("Processing conversation %d: %s", index, title)
            ...
            if not converted_messages:
                if self.verbose:
                    logger.debug("No valid messages found in conversation %d", index)
                return
        ...
        for i, (user_msg, assistant_msg) in enumerate(pairs):
            try:
                ...
                if self.verbose and i % 5 == 0:
                    logger.debug("Stored %d message pairs from conversation", i + 1)
                    
            except Exception as e:
                logger.error("Error storing message pair %d: %s", i, e)
                continue


def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Import ChatGPT conversations to WhisperEngine (Synchronous)')
    ...
    # Run import
    try:
        importer = SyncChatGPTImporter(args.user_id, dry_run=args.dry_run, verbose=args.verbose)
