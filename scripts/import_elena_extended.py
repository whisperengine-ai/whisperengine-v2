...
async def import_response_guidelines(conn: asyncpg.Connection, data: Dict[str, Any]):
    ...
    print(f"  ✅ Imported {len(guidelines)} response guidelines")
...
async def import_conversation_flows(conn: asyncpg.Connection, data: Dict[str, Any]):
    ...
    print(f"  ✅ Imported {len(flows)} conversation flows with directives")
...
async def import_emoji_patterns(conn: asyncpg.Connection, data: Dict[str, Any]):
    ...
    for level, pattern in excitement_levels.items():
        await conn.execute("""
            ...
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        """, CHARACTER_ID, 'excitement_level', f'excitement_{level}', 
             pattern, level, 'high' if level == 'high' else 'medium', pattern)
    ...
    for topic, emojis in topic_patterns.items():
        if isinstance(emojis, list):
            ...
            await conn.execute("""
                ...
                (character_id, pattern_category, pattern_name, emoji_sequence, usage_context, frequency, example_usage)
                VALUES ($1, $2, $3, $4, $5, $6, $7)
            """, CHARACTER_ID, 'topic_specific', topic, emoji_sequence, topic, 'high', 
    ...
    for response_type, pattern in response_types.items():
        await conn.execute("""
            ...
            (character_id, pattern_category, pattern_name, emoji_sequence, usage_context, frequency, example_usage)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        """, CHARACTER_ID, 'response_type', response_type, pattern, response_type, 'high', pattern)
        patterns.append(response_type)
    
    print(f"  ✅ Imported {len(patterns)} emoji patterns")

async def import_ai_scenarios(conn: asyncpg.Connection, data: Dict[str, Any]):
    """Import AI identity handling scenarios"""
    print("\n🤖 Importing AI Identity Scenarios...")
    ...
    if physical:
        await conn.execute("""
            ...
            (character_id, scenario_type, scenario_name, trigger_phrases, response_pattern, 
             tier_1_response, tier_2_response, tier_3_response, example_usage)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        """, CHARACTER_ID, 'physical_interaction', 'physical_meetups', 
             'coffee, meet up, hang out', 'three_tier',
    ...
    if shared:
        await conn.execute("""
            ...
            (character_id, scenario_type, scenario_name, trigger_phrases, response_pattern, 
             tier_1_response, tier_2_response, tier_3_response, example_usage)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        """, CHARACTER_ID, 'activity_participation', 'shared_activities', 
             'explore together, go diving, visit beach', 'three_tier',
    ...
    for scenario_name, response in examples.items():
        await conn.execute("""
            ...
            (character_id, scenario_type, scenario_name, trigger_phrases, response_pattern, 
             tier_1_response, tier_2_response, tier_3_response, example_usage)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        """, CHARACTER_ID, 'example_scenario', scenario_name, '', 'single_response',
             response, '', '', scenario_name.replace('_', ' '))
        scenarios.append(scenario_name)
    
    print(f"  ✅ Imported {len(scenarios)} AI identity scenarios")
...
async def import_cultural_expressions(conn: asyncpg.Connection, data: Dict[str, Any]):
    ...
    for phrase in favorite_phrases:
        ...
        is_spanish = any(spanish_word in phrase.lower() for spanish_word in 
                        ['¡', 'hola', 'amor', 'corazón', 'vida', 'qué', 'ay', 'mi', 'maravilloso', 'increíble'])
    ...
    for pattern in speech_patterns:
        if any(word in pattern.lower() for word in ['spanish', 'amor', 'corazón', 'endearment']):
            await conn.execute("""
                INSERT INTO character_cultural_expressions 
                (character_id, expression_type, expression_value, meaning, usage_context, emotional_context, frequency)
                VALUES ($1, $2, $3, $4, $5, $6, $7)
            """, CHARACTER_ID, 'speech_pattern', pattern, pattern, 'mexican_american', 'affectionate', 'high')
    ...
    if cultural_bg:
        await conn.execute("""
            ...
            (character_id, expression_type, expression_value, meaning, usage_context, emotional_context, frequency)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        """, CHARACTER_ID, 'cultural_background', cultural_bg, 'Heritage and identity', 'mexican_american', 'proud', 'constant')
        expressions.append('cultural_background')
    
    print(f"  ✅ Imported {len(expressions)} cultural expressions")
    ...
    for trait_name, trait_value in trait_mappings:
        if trait_value:
            await conn.execute("""
                ...
                VALUES ($1, $2, $3, $4, $5)
            """, CHARACTER_ID, trait_name, trait_value, 'all_contexts', 
                 f"{trait_name.replace('_', ' ').title()} characteristic")
            traits.append(trait_name)
    
    print(f"  ✅ Imported {len(traits)} voice traits")
...
async def import_emotional_triggers(conn: asyncpg.Connection, data: Dict[str, Any]):
    ...
    for trigger in enthusiasm_triggers:
        await conn.execute("""
            ...
            (character_id, trigger_type, trigger_category, trigger_content, emotional_response, 
             response_intensity, response_examples)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        """, CHARACTER_ID, 'enthusiasm', 'positive', trigger, 'excitement', 'high', 
             'animated_passionate_sharing')
    ...
    for trigger in concern_triggers:
        await conn.execute("""
            ...
            (character_id, trigger_type, trigger_category, trigger_content, emotional_response, 
             response_intensity, response_examples)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        """, CHARACTER_ID, 'concern', 'environmental', trigger, 'worry', 'medium', 
             'gentle_determined_advocacy')
    ...
    for method in support_methods:
        await conn.execute("""
            ...
            (character_id, trigger_type, trigger_category, trigger_content, emotional_response, 
             response_intensity, response_examples)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        """, CHARACTER_ID, 'support', 'caring', method, 'nurturing', 'high', 
             'warm_encouraging_guidance')
        triggers.append(('support', method))
    
    print(f"  ✅ Imported {len(triggers)} emotional triggers")
...
async def import_expertise_domains(conn: asyncpg.Connection, data: Dict[str, Any]):
    ...
    for project in projects:
        await conn.execute("""
            ...
            (character_id, domain_name, expertise_level, domain_description, key_concepts, 
             teaching_approach, passion_level, examples)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        """, CHARACTER_ID, 
             project.get('name', ''),
             'expert',
             ...
             'hands_on_passionate',
             90,
             f"Active research: {project.get('name', '')}")
    ...
    for topic in preferred_topics:
        await conn.execute("""
            ...
            (character_id, domain_name, expertise_level, domain_description, key_concepts, 
             teaching_approach, passion_level, examples)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        """, CHARACTER_ID, topic, 'expert',
             f'Extensive practical experience in {topic}',
             topic,
             'enthusiastic_accessible',
             85,
             f'{topic.replace("_", " ").title()} discussions and education')
        domains.append(topic)
    
    print(f"  ✅ Imported {len(domains)} expertise domains")
...
async def main():
    ...
    try:
        ...
        print("=" * 70)
        print("\n✅ Elena extended import complete!")
