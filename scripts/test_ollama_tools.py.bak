#!/usr/bin/env python3
"""
Test Ollama Tool Calling Compatibility

Tests whether a locally running Ollama model supports the OpenAI-compatible
tool/function calling format required by WhisperEngine.

Usage:
    python scripts/test_ollama_tools.py
    python scripts/test_ollama_tools.py --base-url http://localhost:11434/v1
    python scripts/test_ollama_tools.py --model "llama3.1"
    python scripts/test_ollama_tools.py --stress  # Run stress test with large context
...
class OllamaToolTester:
    def __init__(self, base_url: str, model: Optional[str] = None):
        ...
        self.client = AsyncOpenAI(
            base_url=base_url,
            api_key="ollama"  # Ollama ignores this but requires it
        )
        self.results = []
    ...
    async def get_model_info(self) -> dict:
        ...
        try:
            ...
            if models.data:
                ...
                if self.model:
                    found = next((m for m in models.data if m.id == self.model), None)
                    if found:
                        self._detected_model = found.id
                        return {"id": found.id, "available": True}
                    else:
                        # Fallback: use first available if specified not found? No, better to error or warn.
                        # But for now let's just use the first one if user didn't specify, or warn if they did.
                        pass
            ...
            return {"id": "unknown", "available": False, "error": "No models found in Ollama"}
        except Exception as e:
            return {"id": "error", "error": str(e), "available": False}

    def _get_model_name(self) -> str:
        """Get model name to use for API calls."""
        return self.model or self._detected_model or "llama3.1"
    ...
    async def test_tool_calling(self, test_case: dict) -> dict:
        ...
        try:
            ...
            result["details"]["latency"] = f"{latency:.2f}s"
    ...
    async def run_all_tests(self) -> dict:
        ...
        print("\n" + "="*70)
        print("🦙 OLLAMA TOOL CALLING COMPATIBILITY TEST")
        print("="*70)
        ...
        for result in results:
            ...
            if result["details"].get("latency"):
                print(f"  Latency: {result['details']['latency']}")
...
""" * 20  # Repeat to fill space
    
    # Truncate to desired size
    if len(memory_block) > chars_needed:
        memory_block = memory_block[:chars_needed]
    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"{memory_block}\n\nBased on all that context, do you remember what my cat's name is?"
                }
            ],
            tools=WHISPERENGINE_TOOLS,
            tool_choice="auto",
        ...
        if choice.message.tool_calls:
            tool_call = choice.message.tool_calls[0]
            print(f"  ✅ PASS: Tool called correctly after {context_size} tokens")
            print(f"     Tool: {tool_call.function.name}")
            print(f"     Latency: {latency:.2f}s")
            return {"success": True, "latency": latency}
        else:
            content = choice.message.content or ""
            print(f"  ❌ FAIL: No tool called (model likely got lost in context)")
...
async def test_langchain_tools(base_url: str, model: Optional[str] = None) -> dict:
    """
    Test LangChain's native bind_tools() with Ollama.
...
async def test_all_models(base_url: str, test_langchain: bool = True) -> dict:
    ...
    for i, model in enumerate(models, 1):
        ...
        # Run OpenAI API test
        tester = OllamaToolTester(base_url, model)
...
async def main():
    parser = argparse.ArgumentParser(
        description="Test Ollama tool calling compatibility for WhisperEngine"
    )
    parser.add_argument(
        "--endpoint", "--base-url",
        dest="base_url",
        default="http://localhost:11434/v1",
        help="Ollama API endpoint URL (default: http://localhost:11434/v1)"
    )
    parser.add_argument(
        "--model",
        default=None,
    ...
    if not args.langchain or args.all:
        tester = OllamaToolTester(args.base_url, args.model)
