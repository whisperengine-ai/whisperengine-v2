...
class PreBuiltExecutableBuilder:
    ...
    def download_models(self) -> bool:
        ...
        try:
            ...
            # Check model sizes
            model_size_gb = sum(f.stat().st_size for f in models_dir.rglob('*') if f.is_file()) / (1024**3)
    ...
    def build_executable(self, platform_name: Optional[str] = None) -> Tuple[bool, Optional[Path]]:
        ...
        try:
            ...
            # Check size
            if app_path.is_dir():
                size_gb = sum(f.stat().st_size for f in app_path.rglob('*') if f.is_file()) / (1024**3)
            else:
                size_gb = app_path.stat().st_size / (1024**3)
            
            if size_gb > self.config["max_size_gb"]:
                self.log(f"WARNING: Build size ({size_gb:.1f}GB) exceeds maximum ({self.config['max_size_gb']}GB)", "WARN")
...
def main():
    ...
    parser.add_argument("--test-only", action="store_true",
                       help="Only test existing executable")
