...
- **Memory neighborhood retrieval enhanced**: When searching memories, the system now returns linked memories AND user facts from the graph. This creates richer context—a query about "coffee" now surfaces not just coffee-related memories but associated people, locations, and preferences.
...
- **Dream Journal vs Reverie clarification**: These are TWO SEPARATE features:
  - **Dream Journal** (`dream_journal_graph.py`): Generates first-person dream narratives broadcast to users. Uses DreamJournalAgent with generator-critic loop.
  - **Reverie** (`reverie/graph.py`): Background memory consolidation that links memories together. Invisible to users.
...
- **Memory neighborhoods as associative thinking**: The enhanced retrieval that includes linked memories mirrors associative thinking in humans—one memory triggering related ones. Could observe whether this creates more "natural" conversation flow.

- **Stream + Reverie synergy**: Fast responses (Stream) create more data points; idle consolidation (Reverie) makes sense of them. This creates a learning loop: engage → accumulate → consolidate → engage with richer context.
...
## 📝 Artifacts Created (Dec 11-12)

### Major Features Completed
1. **Phase 2.5.2: The Stream** — Hybrid event-driven architecture with immediate triggers
2. **Phase 2.5.3: Reverie** — Active Idle state replacing Dream system
3. **Memory Neighborhood Retrieval** — Enhanced graph traversal for linked memories + facts

### Documentation
4. **PROJECT_REPORT_20251211.md** — Phase 2.5.3 (Dream) completion report
5. **PROJECT_REPORT_20251211_STREAM.md** — Phase 2.5.2 (Stream) completion report
6. **RELEASE_NOTES_v2.5.md** — Unified Memory release documentation
7. **SPEC-E34-REVERIE.md** — Reverie specification (renamed from Dream)
8. **SPEC-E37-ADAPTIVE_IDENTITY_SELF_EDITING.md** — Renamed from "Soul" for clarity
9. **ADR-012-V2.5_UNIFIED_ARCHITECTURE.md** — Updated with Unified Memory details

### Code
10. **src_v2/agents/reverie/graph.py** — New ReverieGraph (225 lines)
11. **src_v2/agents/daily_life/graph.py** — Major refactoring for bot planning optimization
12. **src_v2/knowledge/manager.py** — Enhanced memory neighborhood retrieval
13. **src_v2/discord/daily_life.py** — Stream triggers + 3-channel exploration
14. **src_v2/config/settings.py** — Deprecated old flags, added watch_channels visibility

### Tests
15. **tests_v2/test_dream_cycle.py** — Reverie verification suite
16. **tests_v2/test_stream_trigger.py** — Stream trigger verification
17. **tests_v2/test_unified_memory_dual_write.py** — Unified Memory integration test
...
- "Dream" → "Reverie" (Active Idle is more than just dreams)
- "Soul" → "Adaptive Identity" (less mystical, more accurate)
- "Graph Memory Unification" → "Unified Memory" (consistent branding)
