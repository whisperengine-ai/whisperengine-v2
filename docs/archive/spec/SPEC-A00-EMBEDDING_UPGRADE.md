...
class EmbeddingService:
    // Initialize with model from settings
    function __init__(model_name=None):
...
```python
// Script to migrate Qdrant collections from 384D to 768D
// Usage: python migrate.py --collection name

function migrate_collection(old_collection, new_collection):
    // 1. Check if old collection exists
    info = qdrant.get_collection(old_collection)
    if not info: return

    // 2. Create new collection with 768 dimensions
    if not qdrant.exists(new_collection):
        qdrant.create_collection(
            name=new_collection,
            vector_size=768
        )

    // 3. Scroll through all points in old collection
    while True:
        points = qdrant.scroll(old_collection, batch_size=100)
        if not points: break

        new_points = []
        for point in points:
            // 4. Re-embed content with new model
            content = point.payload["content"]
            new_vector = embedding_service.embed(content)
        ...
        // 5. Upsert to new collection
        qdrant.upsert(new_collection, new_points)

function main():
    args = parse_args()
    
    // Initialize new 768D embedding model
    embedding_service = EmbeddingService(model="BAAI/bge-base-en-v1.5")
    
    // Get list of collections to migrate
    collections = args.collection or qdrant.get_all_collections()
    
    for collection in collections:
        new_name = collection + "_768d"
        migrate_collection(collection, new_name)
        
        if args.delete_old:
            qdrant.delete(collection)
            print(f"Migrated {collection} to {new_name}")
