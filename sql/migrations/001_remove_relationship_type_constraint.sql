...
-- Note: This migration only applies if user_fact_relationships table exists

-- Check if table exists before attempting modification
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_fact_relationships') THEN
        ...
        RAISE NOTICE 'Migration applied: user_fact_relationships constraint updated';
    ELSE
        RAISE NOTICE 'Migration skipped: user_fact_relationships table does not exist';
    END IF;
END $$;
